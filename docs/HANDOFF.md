...
آخر تحديث: 2026-09-19 · `main` يحوي T0.1–T0.20 وT1.1–T1.43 (PRs #1–#65 مدمجة؛ المرحلة ١ مكتملة) · **المرحلة ٢ بدأت بإذن المالك (2026-09-19)** — خطتها في `docs/PLAN.md` §٦ (T2.1–T2.17) · **PR #66** = T2.1 ORG-01/02 على `phase2/t2.1-org-01-02` — ادمجه بعد خضرة الفحوص الأربعة · التالي: T2.2 (ORG-03 + ORG-04) على `phase2/t2.2-org-03-04`
...
> 1. اقرأ `docs/HANDOFF.md` كاملاً (ترتيب القراءة في §1، الحالة في §2، الأوامر والفخاخ في §3، ما ينتظر المالك في §4، خطوات البدء في §5) ثم `docs/decisions/0005-acc-01-02-frame-conflicts-and-identity.md` §١–§٤٧ (القرارات والافتراضات المسجّلة؛ §٤٦ = ما كشفته بوابة T1.43، §٤٧ = ORG-01/02).
> 2. `git fetch --all` ثم `gh pr list`: إن كان PR #66 (T2.1 ORG-01/02) مفتوحاً — `gh pr checks 66`؛ خضرة الفحوص **الأربعة** (backend، packages، e2e، gate) ⇒ `gh pr merge 66 --merge --delete-branch` (الإذن قائم منذ T1.9 لكل PR أخضر)؛ أحمر ⇒ أصلح على فرعه أولاً. PRs #1–#65 مدمجة.
...
> 5. **المرحلة ٢ جارية بإذن المالك** — الخطة في `docs/PLAN.md` §٦ (16 مهمة + بوابة T2.17) بالترتيب ORG → REP → NOT → PUR → PUB. نفّذ **T2.2** = ORG-03 الفروع وتفاصيلها (4) + ORG-04 قائمة الأجهزة وتفاصيلها (5) على فرع `phase2/t2.2-org-03-04` من `origin/main`: رمز الفرع في الترقيم ثابت بعد أول فاتورة (§٨.٢)؛ الأجهزة بآخر مزامنة والمعلّق المبلَّغ والبادئة وهويتي PWA/أصلي (§٩.٣، §٨.١٣؛ ACC-42، 71، 102). ابنِ على `core.org` (T2.1) وشاشتي `/org/users` و`/org/roles`. الحلقة نفسها لكل شاشة: `frameTextsAll` ← البناء من `@sting/ui-web` فقط بجذر `data-screen`/`data-state` ← Playwright بـ`expectFrame`/`fromFrame` على 390/834/1440 ← commit ← `gh pr create --base main` ← دمج بعد خضرة CI ← التالي؛ ولكل معيار §١٨ جديد ذي أثر مرئي سيناريو في `apps/web/e2e-gate` وصفّ في `docs/evidence/README.md` (أو تأجيله المعلن إلى T2.17). ما ينتظر المالك في §4 لا تفعله بلا مدخلاته.
...
المجاميع بعد T2.1: **pytest 376 · Vitest 348 (+1 todo) · Playwright 933 (311 × 3 مقاسات) · gate 7/7** (Playwright الكامل لم يُشغَّل محلياً في هذه الجلسة — CI هو المرجع؛ البوابة 7/7 محلياً على Django حقيقي).
...
2. **شغّل الفحوص الأربعة في §3** كلٌّ وحده وتأكد من المجاميع (pytest 376 / Vitest 348 (+1 todo) / Playwright 933 / gate 7 — Playwright بـ`--workers=3` وحده؛ تشغيله مع غيره يجوّعه فتسقط اختبارات بمهلات).
3. **اقرأ §1 بالترتيب** (القسم ٣ من الأمر إلزامي كاملاً).
4. **الفرع**: `git checkout -b phase2/t2.2-org-03-04 origin/main` بعد دمج #66.
5. **نفّذ T2.2** = ORG-03 (4) + ORG-04 (5) وفق PLAN.md §٦. ما بقي من المرحلة ١ ينتظر المالك (§4): عتاد الطباعة (ACC-83)، PWA مثبَّت لإثبات ACC-72، أيقونات/مفاتيح/خطوط، وحسم الافتراضات؛ ACC-63/105/109 تُثبت في T2.3/T2.12/T2.17.
