...
آخر تحديث: 2026-09-19 · `main` يحوي T0.1–T0.20 وT1.1–T1.43 وT2.1–T2.6 (PRs #1–#71 مدمجة) · **المرحلة ٢ جارية** (PLAN.md §٦) · PRs مكدّسة: **#72** T2.7 → **#73** T2.8 → **#74** T2.9 REP-05/06 (كل فرع فوق سابقه) — ادمجها بالترتيب بعد خضرة الفحوص الأربعة · التالي: T2.10 (NOT-01 صندوق الوارد + NOT-02 تفضيلات التنبيه) على `phase2/t2.10-not-01-02`
...
> 1. اقرأ `docs/HANDOFF.md` كاملاً (ترتيب القراءة في §1، الحالة في §2، الأوامر والفخاخ في §3، ما ينتظر المالك في §4، خطوات البدء في §5) ثم `docs/decisions/0005-acc-01-02-frame-conflicts-and-identity.md` §١–§٥٥ (القرارات والافتراضات المسجّلة؛ §٤٦ = بوابة T1.43، §٤٧–§٥٢ = ORG-01…10، §٥٣–§٥٥ = REP-01…06).
> 2. `git fetch --all` ثم `gh pr list`: PRs #72 (T2.7) → #73 (T2.8) → #74 (T2.9) مكدّسة كلٌّ فوق سابقه — لكلٍّ بالترتيب: خضرة الفحوص **الأربعة** (backend، packages، e2e، gate) ⇒ `gh pr merge N --merge --delete-branch` (الإذن قائم منذ T1.9 لكل PR أخضر)؛ أحمر ⇒ أصلح على فرعه أولاً. PRs #1–#71 مدمجة.
...
> 5. **المرحلة ٢ جارية بإذن المالك** — الخطة في `docs/PLAN.md` §٦ (16 مهمة + بوابة T2.17) بالترتيب ORG → REP → NOT → PUR → PUB. ORG-01…10 وREP-01…06 مكتملة (T2.1–T2.9). نفّذ **T2.10** = NOT-01 صندوق الوارد والتفاصيل (5 حالات؛ `17-D12#NOT-01`: مصدر الحقيقة خادمي؛ إعادة فحص التخويل عند الرابط العميق ACC-113؛ العرض المنتهي لا يُنبَّه به ACC-110) + NOT-02 تفضيلات التنبيه (4؛ `17-D12#NOT-02`: لكل قناة؛ `offline` يحفظ محلياً ويُرفع)؛ §١١.٥، §١١.٦؛ معايير §١٨ ACC-110، 113؛ WEB-02 يغذّي الوارد ورفض Push لا يعطّل الصندوق — على فرع `phase2/t2.10-not-01-02` من `origin/main` (بعد دمج #72–#74). ابنِ على `core/push.py` (أنواع الإشعارات المغلقة وعناوينها) و`core/home.py::notices` (HOME-02) و`core/audit.py` كمصادر للوارد. الحلقة نفسها لكل شاشة: `frameTextsAll` ← البناء من `@sting/ui-web` فقط بجذر `data-screen`/`data-state` ← Playwright بـ`expectFrame`/`fromFrame` على 390/834/1440 ← commit ← `gh pr create --base main` ← دمج بعد خضرة CI ← التالي؛ ولكل معيار §١٨ جديد ذي أثر مرئي سيناريو في `apps/web/e2e-gate` وصفّ في `docs/evidence/README.md`. ما ينتظر المالك في §4 لا تفعله بلا مدخلاته.
...
المجاميع بعد T2.9: **pytest 395 · Vitest 348 (+1 todo) · Playwright 1026 (342 × 3 مقاسات؛ مواصفات T2.7–T2.9 الجديدة 12/12 كلٌّ محلياً — التشغيل الكامل لم يُعَد بعد T2.7، CI هو المرجع) · gate 8/8** (البوابة 7/7 محلياً على Django حقيقي).
...
1. **تحقق من الحالة**: `git status` نظيف؛ `gh pr list` — #72 → #73 → #74 مكدّسة أو ما بقي منها: ادمجها بالترتيب بعد خضرة الفحوص الأربعة. إن اختلف الوضع، أبلغ المالك قبل المتابعة.
2. **شغّل الفحوص الأربعة في §3** كلٌّ وحده وتأكد من المجاميع (pytest 395 / Vitest 348 (+1 todo) / Playwright 1026 / gate 8 — Playwright بـ`--workers=3` وحده؛ تشغيله مع غيره يجوّعه فتسقط اختبارات بمهلات).
3. **اقرأ §1 بالترتيب** (القسم ٣ من الأمر إلزامي كاملاً).
4. **الفرع**: `git checkout -b phase2/t2.10-not-01-02 origin/main` بعد دمج #72–#74.
5. **نفّذ T2.10** = NOT-01 (5) + NOT-02 (4) وفق PLAN.md §٦. ما بقي من المرحلة ١ ينتظر المالك (§4): عتاد الطباعة (ACC-83)، PWA مثبَّت لإثبات ACC-72، أيقونات/مفاتيح/خطوط، وحسم الافتراضات؛ ACC-63/105/109 تُثبت في T2.3/T2.12/T2.17.
6. بعدها T2.11 … T2.17 بترتيب PLAN.md §٦. لكل شاشة الحلقة نفسها، ولكل معيار مرئي جديد سيناريو في `e2e-gate` وصفّ في ملف الأدلة. عند أي غموض: توقّف، سجّل في `docs/decisions/000N-*.md`، اسأل.
