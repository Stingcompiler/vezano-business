...
آخر تحديث: 2026-09-17 · `main` يحوي T0.1–T0.20 وT1.1–T1.25 (PRs #1–#46 مدمجة) · **PR #47 (T1.26: PTY-07/PTY-09، فرع `phase1/t1.26-pty-07-09`) مفتوح — يُدمج بعد خضرة CI إن لم يكن دُمج** · التالي: T1.27
...
> 2. `git fetch --all` ثم تحقق من `gh pr list`: إن كان PR #47 (T1.26: PTY-07/PTY-09، فرع `phase1/t1.26-pty-07-09`) ما زال مفتوحاً وCI أخضر (`gh pr checks 47`) فادمجه بـ`gh pr merge 47 --merge --delete-branch` (المالك أذن بالدمج بعد خضرة CI منذ T1.9)؛ ثم `git checkout -b phase1/t1.27-pty-08 origin/main`.
...
> 5. نفّذ **T1.27** = PTY-08 طباعة وتصدير ومشاركة الكشف (5 حالات) وفق `docs/PLAN.md` §٣ (الصف T1.27: `04-D2#PTY-08`؛ §١٤.١؛ معيار ACC-85؛ N-01؛ R-06/R-09): معاينة تعرض ما سيخرج؛ قالب تذكير الدين يدوي يراجعه المالك ويرسله بنفسه (داخل `ready`) — لا زر إرسال آلي؛ المشاركة تبيّن حالة التسليم بصدق. الكشف الخادمي جاهز من T1.24 (`GET /api/parties/{id}/statement`) وسطوره تحمل `source_party` بعد الدمج (T1.26). بالحلقة نفسها: `frameTextsAll` ← البناء من `@sting/ui-web` فقط بجذر `data-screen`/`data-state` ← Playwright بـ`expectFrame`/`fromFrame` على 390/834/1440 ← commit ← `gh pr create --base main` ← دمج بعد خضرة CI ← التالي بترتيب PLAN.md.
...
| 3b | `docs/decisions/0005-acc-01-02-frame-conflicts-and-identity.md` | قاعدة تطبيق «بطاقات الحالة» D26، تعارضات D2/D8/D26، نموذج الهوية (Account بعدة عضويات)، وملاحق كل مهمة T1.x (§٥–§٢٩) بافتراضاتها |
...
#47 phase1/t1.26-pty-07-09                → main (مفتوح عند التسليم — **ادمجه أولاً بعد خضرة CI**)
```

**كل ما سبق مدمج في `main` عدا #47.** فرع البداية للمهمة التالية هو `main` بعد دمج #47؛ الفروع القديمة محذوفة. الدمج من المنفّذ مأذون به بعد خضرة CI (منذ T1.9).
...
المجاميع بعد T1.26: **pytest 349 · Vitest 333 (+1 todo) · Playwright 645 (215 × 3 مقاسات)**.
...
1. **تحقق من الحالة**: `git status` نظيف؛ `gh pr list` يطابق الجدول في §2 — PR #47 إن كان مفتوحاً وأخضر فادمجه. إن اختلف الوضع، أبلغ المالك قبل المتابعة.
2. **شغّل الفحوص الثلاثة في §3** وتأكد من المجاميع (pytest 349 / Vitest 333 / Playwright 645 — شغّل Playwright وحده وبـ`--workers=3` على هذا الجهاز؛ تشغيله مع pytest/Vitest يجوّعه فتسقط اختبارات بمهلات). فشلٌ هنا يُبلَّغ ولا يُرقَّع.
3. **اقرأ §1 بالترتيب** (القسم ٣ من الأمر إلزامي كاملاً).
4. **أنشئ الفرع**: `git checkout -b phase1/t1.26-pty-07-09` من `main`.
5. **نفّذ T1.27** = PTY-08 طباعة وتصدير ومشاركة الكشف (5) وفق PLAN.md §٣ (`04-D2#PTY-08`؛ §١٤.١؛ ACC-85؛ N-01): معاينة تعرض ما سيخرج؛ قالب تذكير يدوي داخل `ready` بلا إرسال آلي؛ حالة التسليم بصدق (R-06/R-09). ثم T1.28 بترتيب PLAN.md.
6. بعدها T1.28 … بترتيب PLAN.md: ACC → HOME → CAT → SHIFT → POS → PTY → INV → SYS → WEB → بوابة T1.43. عند أي غموض: توقّف، سجّل في `docs/decisions/000N-*.md`، اسأل.
