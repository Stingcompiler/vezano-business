...
آخر تحديث: 2026-09-18 · `main` يحوي T0.1–T0.20 وT1.1–T1.32 (PRs #1–#53 مدمجة) · **PR #54 (T1.33: INV-10، فرع `phase1/t1.33-inv-10`) مفتوح — يُدمج بعد خضرة CI إن لم يكن دُمج** · التالي: T1.34
...
> 1. اقرأ `docs/HANDOFF.md` كاملاً (ترتيب القراءة في §1، الحالة في §2، الأوامر في §3، الفخاخ في §3، خطوات البدء في §5) ثم `docs/decisions/0005-acc-01-02-frame-conflicts-and-identity.md` §١–§٣٦ (القرارات والافتراضات المسجّلة — الملاحق §٢١ فما بعد لمهام PTY/INV الأخيرة).
> 2. `git fetch --all` ثم تحقق من `gh pr list`: إن كان PR #54 (T1.33: INV-10، فرع `phase1/t1.33-inv-10`) ما زال مفتوحاً وCI أخضر (`gh pr checks 54`) فادمجه بـ`gh pr merge 54 --merge --delete-branch` (المالك أذن بالدمج بعد خضرة CI منذ T1.9)؛ ثم `git checkout -b phase1/t1.34-sys-01-02 origin/main`.
...
> 4. تحقق: `pnpm check` (Vitest 342 + 1 todo) · `cd backend && DATABASE_URL=postgresql:///sting_t115 STING_ENV=test STING_FAULTS_ENABLED=1 uv run pytest -p no:warnings` (358؛ `sting_t115` قاعدة اختبار منفصلة عن `sting_dev` — أنشئها بـ`createdb sting_t115` وطبّق `migrate` عليها إن لم توجد) · `cd apps/web && pnpm exec playwright test --workers=3` (774 = 258 × 3؛ في الخلفية؛ وحده لا مع غيره — يستغرق نحو 25 دقيقة). فشلٌ يُبلَّغ ولا يُرقَّع؛ اختبار يسقط في التشغيل الكامل ويمرّ وحده = حِمل لا عيب (يُذكر في PR).
> 5. نفّذ **T1.34** = SYS-01 مركز الاتصال والمزامنة (6 حالات) + SYS-02 تفاصيل عملية متعثرة (5 حالات) وفق `docs/PLAN.md` §٣ (الصف T1.34: `07-D3#SYS-01`، `16-D11#SYS-02`؛ §١٣.٦، §٨.٣؛ معايير ACC-04، 05، 06، 29، 59، 71، 88): آخر نجاح، المعلّق وعمر أقدمه، المتعارض والمحجور، «هناك شبكة» ≠ «نجح الوصول»؛ تفاصيل عملية متعثرة بمسار إصلاح؛ تعطل Redis/العامل لا يمنع الالتزام؛ ACK لبيع محلي لا يغيّر الرصيد. ابنِ على `listOperationsByState` وحالات العملية في Dexie (`pending/synced/retry/conflicted/quarantined`)، و`pushPending` في `apps/web/src/lib/sync.ts` (نتائجه `applied/idle/retry/…`)، و`SyncIndicator` في `@sting/ui-web`، وعلى `sync_epoch`/`epoch_mismatch`؛ لا تخترع شاشة غير مرسومة. بالحلقة نفسها: `frameTextsAll` ← البناء من `@sting/ui-web` فقط بجذر `data-screen`/`data-state` ← Playwright بـ`expectFrame`/`fromFrame` على 390/834/1440 ← commit ← `gh pr create --base main` ← دمج بعد خضرة CI ← التالي بترتيب PLAN.md.
...
المجاميع بعد T1.33: **pytest 358 · Vitest 342 (+1 todo) · Playwright 774 (258 × 3 مقاسات)**.
...
1. **تحقق من الحالة**: `git status` نظيف؛ `gh pr list` يطابق الجدول في §2 — PR #54 إن كان مفتوحاً وأخضر فادمجه. إن اختلف الوضع، أبلغ المالك قبل المتابعة.
2. **شغّل الفحوص الثلاثة في §3** وتأكد من المجاميع (pytest 358 / Vitest 342 / Playwright 774 — شغّل Playwright وحده وبـ`--workers=3` على هذا الجهاز؛ تشغيله مع pytest/Vitest يجوّعه فتسقط اختبارات بمهلات). فشلٌ هنا يُبلَّغ ولا يُرقَّع.
3. **اقرأ §1 بالترتيب** (القسم ٣ من الأمر إلزامي كاملاً).
4. **أنشئ الفرع**: `git checkout -b phase1/t1.34-sys-01-02` من `origin/main`.
5. **نفّذ T1.34** = SYS-01 (6) + SYS-02 (5) وفق PLAN.md §٣ (`07-D3#SYS-01`، `16-D11#SYS-02`؛ §١٣.٦، §٨.٣): مركز الاتصال والمزامنة وتفاصيل عملية متعثرة بمسار إصلاح. ثم T1.35 بترتيب PLAN.md.
6. بعدها T1.35 … بترتيب PLAN.md: ACC → HOME → CAT → SHIFT → POS → PTY → INV → SYS → WEB → بوابة T1.43. عند أي غموض: توقّف، سجّل في `docs/decisions/000N-*.md`، اسأل.
