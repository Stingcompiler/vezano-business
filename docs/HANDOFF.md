...
آخر تحديث: 2026-09-19 · `main` يحوي T0.1–T0.20 وT1.1–T1.43 (PRs #1–#65 مدمجة؛ المرحلة ١ مكتملة) · **المرحلة ٢ جارية** (PLAN.md §٦) · PRs مكدّسة: **#66** T2.1 → **#67** T2.2 → **#68** T2.3 ORG-05 (كل فرع فوق سابقه) — ادمجها بالترتيب بعد خضرة الفحوص الأربعة · التالي: T2.4 (ORG-06 الاشتراك والباقات + ORG-08 انتهاء الاشتراك) على `phase2/t2.4-org-06-08`
...
> 1. اقرأ `docs/HANDOFF.md` كاملاً (ترتيب القراءة في §1، الحالة في §2، الأوامر والفخاخ في §3، ما ينتظر المالك في §4، خطوات البدء في §5) ثم `docs/decisions/0005-acc-01-02-frame-conflicts-and-identity.md` §١–§٤٩ (القرارات والافتراضات المسجّلة؛ §٤٦ = بوابة T1.43، §٤٧ = ORG-01/02، §٤٨ = ORG-03/04، §٤٩ = ORG-05).
> 2. `git fetch --all` ثم `gh pr list`: PRs #66 (T2.1) → #67 (T2.2) → #68 (T2.3) مكدّسة كلٌّ فوق سابقه — لكلٍّ بالترتيب: خضرة الفحوص **الأربعة** (backend، packages، e2e، gate) ⇒ `gh pr merge N --merge --delete-branch` (الإذن قائم منذ T1.9 لكل PR أخضر)؛ أحمر ⇒ أصلح على فرعه أولاً. PRs #1–#65 مدمجة.
...
> 5. **المرحلة ٢ جارية بإذن المالك** — الخطة في `docs/PLAN.md` §٦ (16 مهمة + بوابة T2.17) بالترتيب ORG → REP → NOT → PUR → PUB. نفّذ **T2.4** = ORG-06 الاشتراك والباقات (4: ready/loading/permission_denied/expired؛ F17) + ORG-08 انتهاء الاشتراك (3: ready/expired/permission_denied) على فرع `phase2/t2.4-org-06-08` من `origin/main` (بعد دمج #66–#68): نموذج الخصائص بمفاتيح §١١.١ (لا إعلان مضلل ACC-104)، انتهاء الاشتراك = قراءة فقط + تصدير + إغلاق الورديات المفتوحة ولا حجز بيانات (§١١.٢؛ ACC-80/81/82)، الباقات §١٦.١؛ حدّ أجهزة الباقة يغذّي ORG-04 (`device_limit`). ابنِ على `core/org*.py` وشاشات `/org/*`. الحلقة نفسها لكل شاشة: `frameTextsAll` ← البناء من `@sting/ui-web` فقط بجذر `data-screen`/`data-state` ← Playwright بـ`expectFrame`/`fromFrame` على 390/834/1440 ← commit ← `gh pr create --base main` ← دمج بعد خضرة CI ← التالي؛ ولكل معيار §١٨ جديد ذي أثر مرئي سيناريو في `apps/web/e2e-gate` وصفّ في `docs/evidence/README.md` (ACC-80–82 مطلوبة في T2.17 على الأقل). ما ينتظر المالك في §4 لا تفعله بلا مدخلاته.
...
المجاميع بعد T2.3: **pytest 380 · Vitest 348 (+1 todo) · Playwright 957 (319 × 3 مقاسات) · gate 8/8** (Playwright الكامل لم يُشغَّل محلياً في هذه الجلسة — CI هو المرجع؛ البوابة 7/7 محلياً على Django حقيقي).
...
2. **شغّل الفحوص الأربعة في §3** كلٌّ وحده وتأكد من المجاميع (pytest 380 / Vitest 348 (+1 todo) / Playwright 957 / gate 8 — Playwright بـ`--workers=3` وحده؛ تشغيله مع غيره يجوّعه فتسقط اختبارات بمهلات).
3. **اقرأ §1 بالترتيب** (القسم ٣ من الأمر إلزامي كاملاً).
4. **الفرع**: `git checkout -b phase2/t2.4-org-06-08 origin/main` بعد دمج #66–#68.
5. **نفّذ T2.4** = ORG-06 (4) + ORG-08 (3) وفق PLAN.md §٦. ما بقي من المرحلة ١ ينتظر المالك (§4): عتاد الطباعة (ACC-83)، PWA مثبَّت لإثبات ACC-72، أيقونات/مفاتيح/خطوط، وحسم الافتراضات؛ ACC-63/105/109 تُثبت في T2.3/T2.12/T2.17.
