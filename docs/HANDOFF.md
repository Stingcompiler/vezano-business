...
آخر تحديث: 2026-09-18 · `main` يحوي T0.1–T0.20 وT1.1–T1.36 (PRs #1–#57 مدمجة) · **PR #58 (T1.37: SYS-05/SYS-06، فرع `phase1/t1.37-sys-05-06`) مفتوح — يُدمج بعد خضرة CI إن لم يكن دُمج** · التالي: T1.38
...
> 1. اقرأ `docs/HANDOFF.md` كاملاً (ترتيب القراءة في §1، الحالة في §2، الأوامر في §3، الفخاخ في §3، خطوات البدء في §5) ثم `docs/decisions/0005-acc-01-02-frame-conflicts-and-identity.md` §١–§٤٠ (القرارات والافتراضات المسجّلة — الملاحق §٢١ فما بعد لمهام PTY/INV الأخيرة).
> 2. `git fetch --all` ثم تحقق من `gh pr list`: إن كان PR #58 (T1.37: SYS-05/SYS-06، فرع `phase1/t1.37-sys-05-06`) ما زال مفتوحاً وCI أخضر (`gh pr checks 58`) فادمجه بـ`gh pr merge 58 --merge --delete-branch` (المالك أذن بالدمج بعد خضرة CI منذ T1.9)؛ ثم `git checkout -b phase1/t1.38-sys-07-08 origin/main`.
...
> 4. تحقق: `pnpm check` (Vitest 347 + 1 todo) · `cd backend && DATABASE_URL=postgresql:///sting_t115 STING_ENV=test STING_FAULTS_ENABLED=1 uv run pytest -p no:warnings` (364؛ `sting_t115` قاعدة اختبار منفصلة عن `sting_dev` — أنشئها بـ`createdb sting_t115` وطبّق `migrate` عليها إن لم توجد) · `cd apps/web && pnpm exec playwright test --workers=3` (840 = 280 × 3؛ في الخلفية؛ وحده لا مع غيره — يستغرق نحو 25 دقيقة). فشلٌ يُبلَّغ ولا يُرقَّع؛ اختبار يسقط في التشغيل الكامل ويمرّ وحده = حِمل لا عيب (يُذكر في PR).
> 5. نفّذ **T1.38** = SYS-07 استرداد جهاز مسحوب إلى الحجر (5 حالات) + SYS-08 تغيّر جيل الخادم (4 حالات) وفق `docs/PLAN.md` §٣ (الصف T1.38: `19-D14#SYS-07/08`؛ §٩.٣، §٨.١٢؛ معايير ACC-55، 56، 57، 64): اعتماد خادمي مقيّد لا هوية من الحمولة؛ عند تغيّر الجيل: حفظ المحلي، عزل أرقام الجيل القديم، إعادة التهيئة والمصالحة، إهمال الردود المتأخرة (ACK من الجيل السابق يُهمل)؛ إعادة رفع أحداث فقدها الخادم بهوياتها الأصلية. ابنِ على `sync_epoch`/`epoch_mismatch` في `pusher.ts` و`lib/sync.ts` (`halt`)، و`SyncState.sync_epoch` وبوابة `bootstrap` (ACC-05)، و`devices/register|renew|verifiers` وحالات الجلسات (ACC-08/09)، ومحتوى لوحة 19-D14 لـSYS-08 (G-17/G-18، «المعلَّق ليس مفقوداً»، مؤكَّد/معلَّق/يحتاجك/محسوم)؛ لا تخترع شاشة غير مرسومة. بالحلقة نفسها: `frameTextsAll` ← البناء من `@sting/ui-web` فقط بجذر `data-screen`/`data-state` ← Playwright بـ`expectFrame`/`fromFrame` على 390/834/1440 ← commit ← `gh pr create --base main` ← دمج بعد خضرة CI ← التالي بترتيب PLAN.md.
...
المجاميع بعد T1.37: **pytest 364 · Vitest 347 (+1 todo) · Playwright 840 (280 × 3 مقاسات)**.
...
| تحت Dexie الحقيقي (Chrome) أي `await` لمساعد `async` داخل `storage.transaction` يرمي `PrematureCommitError` — حتى لو لم يَعُد إلا على `tx.*` | اكتب على `tx` مباشرةً داخل المُسقِط، أو سلسلة `.then` غير async على وعد من `tx` (مثل `logAttempt` في `pusher.ts`). Vitest (fake-indexeddb) لا يكشفه — Playwright يكشفه في كل حفظ |
...
1. **تحقق من الحالة**: `git status` نظيف؛ `gh pr list` يطابق الجدول في §2 — PR #58 إن كان مفتوحاً وأخضر فادمجه. إن اختلف الوضع، أبلغ المالك قبل المتابعة.
2. **شغّل الفحوص الثلاثة في §3** وتأكد من المجاميع (pytest 364 / Vitest 347 / Playwright 840 — شغّل Playwright وحده وبـ`--workers=3` على هذا الجهاز؛ تشغيله مع pytest/Vitest يجوّعه فتسقط اختبارات بمهلات). فشلٌ هنا يُبلَّغ ولا يُرقَّع.
3. **اقرأ §1 بالترتيب** (القسم ٣ من الأمر إلزامي كاملاً).
4. **أنشئ الفرع**: `git checkout -b phase1/t1.38-sys-07-08` من `origin/main`.
5. **نفّذ T1.38** = SYS-07 (5) + SYS-08 (4) وفق PLAN.md §٣ (`19-D14#SYS-07/08`؛ §٩.٣، §٨.١٢): استرداد جهاز محجور باعتماد خادمي، وتغيّر جيل الخادم بعزل الردود القديمة والمصالحة. ثم T1.39 بترتيب PLAN.md.
6. بعدها T1.39 … بترتيب PLAN.md: ACC → HOME → CAT → SHIFT → POS → PTY → INV → SYS → WEB → بوابة T1.43. عند أي غموض: توقّف، سجّل في `docs/decisions/000N-*.md`، اسأل.
