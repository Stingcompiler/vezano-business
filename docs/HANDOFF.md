...
آخر تحديث: 2026-09-18 · `main` يحوي T0.1–T0.20 وT1.1–T1.38 (PRs #1–#59 مدمجة) · **PR #60 (T1.39: SYS-09/SYS-10، فرع `phase1/t1.39-sys-09-10`) مفتوح — يُدمج بعد خضرة CI إن لم يكن دُمج** · التالي: T1.40
...
> 1. اقرأ `docs/HANDOFF.md` كاملاً (ترتيب القراءة في §1، الحالة في §2، الأوامر في §3، الفخاخ في §3، خطوات البدء في §5) ثم `docs/decisions/0005-acc-01-02-frame-conflicts-and-identity.md` §١–§٤٢ (القرارات والافتراضات المسجّلة — الملاحق §٢١ فما بعد لمهام PTY/INV الأخيرة).
> 2. `git fetch --all` ثم تحقق من `gh pr list`: إن كان PR #60 (T1.39: SYS-09/SYS-10، فرع `phase1/t1.39-sys-09-10`) ما زال مفتوحاً وCI أخضر (`gh pr checks 60`) فادمجه بـ`gh pr merge 60 --merge --delete-branch` (المالك أذن بالدمج بعد خضرة CI منذ T1.9)؛ ثم `git checkout -b phase1/t1.40-web-01 origin/main`.
...
> 4. تحقق: `pnpm check` (Vitest 348 + 1 todo) · `cd backend && DATABASE_URL=postgresql:///sting_t115 STING_ENV=test STING_FAULTS_ENABLED=1 uv run pytest -p no:warnings` (369؛ `sting_t115` قاعدة اختبار منفصلة عن `sting_dev` — أنشئها بـ`createdb sting_t115` وطبّق `migrate` عليها إن لم توجد) · `cd apps/web && pnpm exec playwright test --workers=3` (867 = 289 × 3؛ في الخلفية؛ وحده لا مع غيره — يستغرق نحو 25 دقيقة). فشلٌ يُبلَّغ ولا يُرقَّع؛ اختبار يسقط في التشغيل الكامل ويمرّ وحده = حِمل لا عيب (يُذكر في PR).
> 5. نفّذ **T1.40** = WEB-01 تثبيت PWA أو عدم دعمه (4 حالات) وفق `docs/PLAN.md` §٣ (الصف T1.40: `09-D5#WEB-01`؛ §١٢.٥، §١٣.٢؛ معايير ACC-76، 93، 95): iOS إضافة للشاشة الرئيسية (§١١.٦)؛ Service Worker كامل — تخزين shell والأصول والخطوط، تحديث آمن لا يستبدل العامل أثناء حفظ معلّق (اربطه بـSYS-09: `countPending` و`app.update`)، فصل نطاق بوابة الزبون، لا cache-all لردود API؛ تثبيت وتشغيل بارد ثم تحديث SW دون فقد المعلّق. ابنِ على `apps/web` (Next 15 — SW يدوي في `public/` وتسجيله من `root-client.tsx`)، و`requestPersistentStorage` (ACC-05)، و`/api/app/update`؛ لا تخترع شاشة غير مرسومة. بالحلقة نفسها: `frameTextsAll` ← البناء من `@sting/ui-web` فقط بجذر `data-screen`/`data-state` ← Playwright بـ`expectFrame`/`fromFrame` على 390/834/1440 ← commit ← `gh pr create --base main` ← دمج بعد خضرة CI ← التالي بترتيب PLAN.md.
...
المجاميع بعد T1.39: **pytest 369 · Vitest 348 (+1 todo) · Playwright 867 (289 × 3 مقاسات)**.
...
1. **تحقق من الحالة**: `git status` نظيف؛ `gh pr list` يطابق الجدول في §2 — PR #60 إن كان مفتوحاً وأخضر فادمجه. إن اختلف الوضع، أبلغ المالك قبل المتابعة.
2. **شغّل الفحوص الثلاثة في §3** وتأكد من المجاميع (pytest 369 / Vitest 348 / Playwright 867 — شغّل Playwright وحده وبـ`--workers=3` على هذا الجهاز؛ تشغيله مع pytest/Vitest يجوّعه فتسقط اختبارات بمهلات). فشلٌ هنا يُبلَّغ ولا يُرقَّع.
3. **اقرأ §1 بالترتيب** (القسم ٣ من الأمر إلزامي كاملاً).
4. **أنشئ الفرع**: `git checkout -b phase1/t1.40-web-01` من `origin/main`.
5. **نفّذ T1.40** = WEB-01 (4) وفق PLAN.md §٣ (`09-D5#WEB-01`؛ §١٢.٥، §١٣.٢): تثبيت PWA أو عدم دعمه، Service Worker كامل بتحديث آمن لا يفقد المعلّق. ثم T1.41 بترتيب PLAN.md.
6. بعدها T1.41 … بترتيب PLAN.md: ACC → HOME → CAT → SHIFT → POS → PTY → INV → SYS → WEB → بوابة T1.43. عند أي غموض: توقّف، سجّل في `docs/decisions/000N-*.md`، اسأل.
