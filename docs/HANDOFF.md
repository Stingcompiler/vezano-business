...
آخر تحديث: 2026-09-19 · `main` يحوي T0.1–T0.20 وT1.1–T1.43 وT2.1–T2.3 (PRs #1–#68 مدمجة) · **المرحلة ٢ جارية** (PLAN.md §٦) · PRs مكدّسة: **#69** T2.4 → **#70** T2.5 → **#71** T2.6 ORG-09/10 (كل فرع فوق سابقه) — ادمجها بالترتيب بعد خضرة الفحوص الأربعة · التالي: T2.7 (REP-01 تقرير المبيعات + REP-02 تقرير الذمم) على `phase2/t2.7-rep-01-02`
...
> 1. اقرأ `docs/HANDOFF.md` كاملاً (ترتيب القراءة في §1، الحالة في §2، الأوامر والفخاخ في §3، ما ينتظر المالك في §4، خطوات البدء في §5) ثم `docs/decisions/0005-acc-01-02-frame-conflicts-and-identity.md` §١–§٥٢ (القرارات والافتراضات المسجّلة؛ §٤٦ = بوابة T1.43، §٤٧–§٥٢ = ORG-01…10).
> 2. `git fetch --all` ثم `gh pr list`: PRs #69 (T2.4) → #70 (T2.5) → #71 (T2.6) مكدّسة كلٌّ فوق سابقه — لكلٍّ بالترتيب: خضرة الفحوص **الأربعة** (backend، packages، e2e، gate) ⇒ `gh pr merge N --merge --delete-branch` (الإذن قائم منذ T1.9 لكل PR أخضر)؛ أحمر ⇒ أصلح على فرعه أولاً. PRs #1–#68 مدمجة.
...
> 5. **المرحلة ٢ جارية بإذن المالك** — الخطة في `docs/PLAN.md` §٦ (16 مهمة + بوابة T2.17) بالترتيب ORG → REP → NOT → PUR → PUB. ORG-01…10 مكتملة (T2.1–T2.6). نفّذ **T2.7** = REP-01 تقرير المبيعات (6 حالات؛ `16-D11#REP-01`: فترة/فرع، `pending_sync` يشمل معلّق هذا الجهاز موسوماً، `stale` بوقت التغطية) + REP-02 تقرير الذمم (5؛ `07-D3#REP-02`: بلا أعمار ديون G-15؛ آخر سداد وأقدم حركة غير مسدَّدة تاريخاً) على فرع `phase2/t2.7-rep-01-02` من `origin/main` (بعد دمج #69–#71)؛ §٧.٦، §١٤.١؛ معيار §١٨ ACC-79؛ الأرقام تطابق HOME-01 وPTY-05 لنفس المدى؛ التقارير المتقدمة تتبع `subscription.has_feature` بعد الانتهاء (§١١.٢). الحلقة نفسها لكل شاشة: `frameTextsAll` ← البناء من `@sting/ui-web` فقط بجذر `data-screen`/`data-state` ← Playwright بـ`expectFrame`/`fromFrame` على 390/834/1440 ← commit ← `gh pr create --base main` ← دمج بعد خضرة CI ← التالي؛ ولكل معيار §١٨ جديد ذي أثر مرئي سيناريو في `apps/web/e2e-gate` وصفّ في `docs/evidence/README.md`. ما ينتظر المالك في §4 لا تفعله بلا مدخلاته.
...
المجاميع بعد T2.6: **pytest 389 · Vitest 348 (+1 todo) · Playwright 990 (330 × 3 مقاسات؛ 974 ناجح + 16 متخطّى بشرط Chromium) · gate 8/8** (البوابة 7/7 محلياً على Django حقيقي).
...
1. **تحقق من الحالة**: `git status` نظيف؛ `gh pr list` — #69 → #70 → #71 مكدّسة أو ما بقي منها: ادمجها بالترتيب بعد خضرة الفحوص الأربعة. إن اختلف الوضع، أبلغ المالك قبل المتابعة.
2. **شغّل الفحوص الأربعة في §3** كلٌّ وحده وتأكد من المجاميع (pytest 389 / Vitest 348 (+1 todo) / Playwright 990 / gate 8 — Playwright بـ`--workers=3` وحده؛ تشغيله مع غيره يجوّعه فتسقط اختبارات بمهلات).
3. **اقرأ §1 بالترتيب** (القسم ٣ من الأمر إلزامي كاملاً).
4. **الفرع**: `git checkout -b phase2/t2.7-rep-01-02 origin/main` بعد دمج #69–#71.
5. **نفّذ T2.7** = REP-01 (6) + REP-02 (5) وفق PLAN.md §٦. ما بقي من المرحلة ١ ينتظر المالك (§4): عتاد الطباعة (ACC-83)، PWA مثبَّت لإثبات ACC-72، أيقونات/مفاتيح/خطوط، وحسم الافتراضات؛ ACC-63/105/109 تُثبت في T2.3/T2.12/T2.17.
6. بعدها T2.8 … T2.17 بترتيب PLAN.md §٦. لكل شاشة الحلقة نفسها، ولكل معيار مرئي جديد سيناريو في `e2e-gate` وصفّ في ملف الأدلة. عند أي غموض: توقّف، سجّل في `docs/decisions/000N-*.md`، اسأل.
