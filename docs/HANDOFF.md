...
آخر تحديث: 2026-09-17 · `main` يحوي T0.1–T0.20 وT1.1–T1.24 (PRs #1–#45 مدمجة) · **PR #46 (T1.25: PTY-06، فرع `phase1/t1.25-pty-06`) مفتوح — يُدمج بعد خضرة CI إن لم يكن دُمج** · التالي: T1.26
...
> 2. `git fetch --all` ثم تحقق من `gh pr list`: إن كان PR #46 (T1.25: PTY-06، فرع `phase1/t1.25-pty-06`) ما زال مفتوحاً وCI أخضر (`gh pr checks 46`) فادمجه بـ`gh pr merge 46 --merge --delete-branch` (المالك أذن بالدمج بعد خضرة CI منذ T1.9)؛ ثم `git checkout -b phase1/t1.26-pty-07-09 origin/main`.
...
> 5. نفّذ **T1.26** = PTY-07 دمج الأطراف (5 حالات) + PTY-09 تصحيح تاريخ الأعمال (4 حالات) وفق `docs/PLAN.md` §٣ (الصف T1.26: `04-D2#PTY-07/09`؛ §٧.٥–٧.٦؛ R-03؛ معيار ACC-78): الدمج أونلاين بصلاحية مالك بمعاينة الرصيدين ومنع الحلقات وخريطة هوية (الأصل يحتفظ بـ`party_id`؛ كل حركة تبقى منسوبة لاسمها الأصلي)، تراجع بحدث جديد؛ `DateCorrection` مستند مستقل يُغيّر `business_date` تقريرياً دون تعديل الأصل. «دمج بتأكيد مزدوج» في PTY-03 معطّل اليوم — فعّله. بالحلقة نفسها: `frameTextsAll` ← البناء من `@sting/ui-web` فقط بجذر `data-screen`/`data-state` ← Playwright بـ`expectFrame`/`fromFrame` على 390/834/1440 ← commit ← `gh pr create --base main` ← دمج بعد خضرة CI ← التالي بترتيب PLAN.md.
...
| 3b | `docs/decisions/0005-acc-01-02-frame-conflicts-and-identity.md` | قاعدة تطبيق «بطاقات الحالة» D26، تعارضات D2/D8/D26، نموذج الهوية (Account بعدة عضويات)، وملاحق كل مهمة T1.x (§٥–§٢٨) بافتراضاتها |
...
#46 phase1/t1.25-pty-06                   → main (مفتوح عند التسليم — **ادمجه أولاً بعد خضرة CI**)
```

**كل ما سبق مدمج في `main` عدا #46.** فرع البداية للمهمة التالية هو `main` بعد دمج #46؛ الفروع القديمة محذوفة. الدمج من المنفّذ مأذون به بعد خضرة CI (منذ T1.9).
...
المجاميع بعد T1.25: **pytest 347 · Vitest 333 (+1 todo) · Playwright 618 (206 × 3 مقاسات)**.
...
1. **تحقق من الحالة**: `git status` نظيف؛ `gh pr list` يطابق الجدول في §2 — PR #46 إن كان مفتوحاً وأخضر فادمجه. إن اختلف الوضع، أبلغ المالك قبل المتابعة.
2. **شغّل الفحوص الثلاثة في §3** وتأكد من المجاميع (pytest 347 / Vitest 333 / Playwright 618 — شغّل Playwright وحده وبـ`--workers=3` على هذا الجهاز؛ تشغيله مع pytest/Vitest يجوّعه فتسقط اختبارات بمهلات). فشلٌ هنا يُبلَّغ ولا يُرقَّع.
3. **اقرأ §1 بالترتيب** (القسم ٣ من الأمر إلزامي كاملاً).
4. **أنشئ الفرع**: `git checkout -b phase1/t1.26-pty-07-09` من `main`.
5. **نفّذ T1.26** = PTY-07 الدمج (5) + PTY-09 تصحيح التاريخ (4) وفق PLAN.md §٣ (`04-D2#PTY-07/09`؛ §٧.٥–٧.٦؛ R-03؛ ACC-78): دمج أونلاين بصلاحية مالك بمعاينة الرصيدين ومنع الحلقات وخريطة هوية، تراجع بحدث جديد؛ `DateCorrection` مستند مستقل. ثم T1.27 بترتيب PLAN.md.
6. بعدها T1.27 … بترتيب PLAN.md: ACC → HOME → CAT → SHIFT → POS → PTY → INV → SYS → WEB → بوابة T1.43. عند أي غموض: توقّف، سجّل في `docs/decisions/000N-*.md`، اسأل.
