...
آخر تحديث: 2026-09-19 · `main` يحوي T0.1–T0.20 وT1.1–T1.43 وT2.1–T2.6 (PRs #1–#71 مدمجة) · **المرحلة ٢ جارية** (PLAN.md §٦) · PRs مكدّسة: **#72** T2.7 → **#73** T2.8 REP-03/04 (الثاني فوق الأول) — ادمجها بالترتيب بعد خضرة الفحوص الأربعة · التالي: T2.9 (REP-05 + REP-06) على `phase2/t2.9-rep-05-06`
...
> 1. اقرأ `docs/HANDOFF.md` كاملاً (ترتيب القراءة في §1، الحالة في §2، الأوامر والفخاخ في §3، ما ينتظر المالك في §4، خطوات البدء في §5) ثم `docs/decisions/0005-acc-01-02-frame-conflicts-and-identity.md` §١–§٥٤ (القرارات والافتراضات المسجّلة؛ §٤٦ = بوابة T1.43، §٤٧–§٥٢ = ORG-01…10، §٥٣–§٥٤ = REP-01…04).
> 2. `git fetch --all` ثم `gh pr list`: PRs #72 (T2.7) → #73 (T2.8) مكدّسان — لكلٍّ بالترتيب: خضرة الفحوص **الأربعة** (backend، packages، e2e، gate) ⇒ `gh pr merge N --merge --delete-branch` (الإذن قائم منذ T1.9 لكل PR أخضر)؛ أحمر ⇒ أصلح على فرعه أولاً. PRs #1–#71 مدمجة.
...
> 5. **المرحلة ٢ جارية بإذن المالك** — الخطة في `docs/PLAN.md` §٦ (16 مهمة + بوابة T2.17) بالترتيب ORG → REP → NOT → PUR → PUB. ORG-01…10 وREP-01…04 مكتملة (T2.1–T2.8). نفّذ **T2.9** = REP-05 الهامش والمقارنة بين الفروع (4 حالات؛ `16-D11#REP-05`: `phase_locked` بنصّه حتى سياسة التكلفة — G-03، ACC-90؛ لا رقم ربح قبل التفعيل) + REP-06 تصدير تقرير ومعاينته (5؛ `19-D14#REP-06`: HTML/CSV بحدّ معلن كما PTY-08، حدّ 12 شهراً بالبدائل الثلاثة، `server_error` يبقي المعاينة؛ التصدير يُحصى ويُحدّ)؛ §٧.٧، §١٤.١؛ معيار §١٨ ACC-90` على فرع `phase2/t2.9-rep-05-06` من `origin/main` (بعد دمج #72–#73). ابنِ على `core/report_views.py` ونمط `completeness`/`?export=csv` و`features/reports/*` (REP-05 التكلفة `phase_locked` حتى تُعتمد سياسة G-03؛ REP-06 حدّ التصدير 12 شهراً بالبدائل الثلاثة كما في 19-D14). الحلقة نفسها لكل شاشة: `frameTextsAll` ← البناء من `@sting/ui-web` فقط بجذر `data-screen`/`data-state` ← Playwright بـ`expectFrame`/`fromFrame` على 390/834/1440 ← commit ← `gh pr create --base main` ← دمج بعد خضرة CI ← التالي؛ ولكل معيار §١٨ جديد ذي أثر مرئي سيناريو في `apps/web/e2e-gate` وصفّ في `docs/evidence/README.md`. ما ينتظر المالك في §4 لا تفعله بلا مدخلاته.
...
المجاميع بعد T2.8: **pytest 393 · Vitest 348 (+1 todo) · Playwright 1014 (338 × 3 مقاسات؛ المواصفتان الجديدتان 12/12 و12/12 محلياً — التشغيل الكامل لم يُعَد في هذه الجلسة، CI هو المرجع) · gate 8/8** (البوابة 7/7 محلياً على Django حقيقي).
...
1. **تحقق من الحالة**: `git status` نظيف؛ `gh pr list` — #72 → #73 مكدّسان أو ما بقي منهما: ادمجها بالترتيب بعد خضرة الفحوص الأربعة. إن اختلف الوضع، أبلغ المالك قبل المتابعة.
2. **شغّل الفحوص الأربعة في §3** كلٌّ وحده وتأكد من المجاميع (pytest 393 / Vitest 348 (+1 todo) / Playwright 1014 / gate 8 — Playwright بـ`--workers=3` وحده؛ تشغيله مع غيره يجوّعه فتسقط اختبارات بمهلات).
3. **اقرأ §1 بالترتيب** (القسم ٣ من الأمر إلزامي كاملاً).
4. **الفرع**: `git checkout -b phase2/t2.9-rep-05-06 origin/main` بعد دمج #72–#73.
5. **نفّذ T2.9** = REP-05 + REP-06 وفق PLAN.md §٦. ما بقي من المرحلة ١ ينتظر المالك (§4): عتاد الطباعة (ACC-83)، PWA مثبَّت لإثبات ACC-72، أيقونات/مفاتيح/خطوط، وحسم الافتراضات؛ ACC-63/105/109 تُثبت في T2.3/T2.12/T2.17.
6. بعدها T2.10 … T2.17 بترتيب PLAN.md §٦. لكل شاشة الحلقة نفسها، ولكل معيار مرئي جديد سيناريو في `e2e-gate` وصفّ في ملف الأدلة. عند أي غموض: توقّف، سجّل في `docs/decisions/000N-*.md`، اسأل.
