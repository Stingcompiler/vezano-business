...
آخر تحديث: 2026-09-19 · `main` يحوي T0.1–T0.20 وT1.1–T1.43 وT2.1 (PRs #1–#66 مدمجة) · **المرحلة ٢ جارية** (PLAN.md §٦) · PRs مكدّسة: **#67** T2.2 → **#68** T2.3 → **#69** T2.4 ORG-06/08 (كل فرع فوق سابقه) — ادمجها بالترتيب بعد خضرة الفحوص الأربعة · التالي: T2.5 (ORG-07 إثبات تحويل الاشتراك ومراجعته) على `phase2/t2.5-org-07`
...
> 1. اقرأ `docs/HANDOFF.md` كاملاً (ترتيب القراءة في §1، الحالة في §2، الأوامر والفخاخ في §3، ما ينتظر المالك في §4، خطوات البدء في §5) ثم `docs/decisions/0005-acc-01-02-frame-conflicts-and-identity.md` §١–§٥٠ (القرارات والافتراضات المسجّلة؛ §٤٦ = بوابة T1.43، §٤٧–§٥٠ = ORG-01…08).
> 2. `git fetch --all` ثم `gh pr list`: PRs #67 (T2.2) → #68 (T2.3) → #69 (T2.4) مكدّسة كلٌّ فوق سابقه — لكلٍّ بالترتيب: خضرة الفحوص **الأربعة** (backend، packages، e2e، gate) ⇒ `gh pr merge N --merge --delete-branch` (الإذن قائم منذ T1.9 لكل PR أخضر)؛ أحمر ⇒ أصلح على فرعه أولاً. PRs #1–#66 مدمجة.
...
> 5. **المرحلة ٢ جارية بإذن المالك** — الخطة في `docs/PLAN.md` §٦ (16 مهمة + بوابة T2.17) بالترتيب ORG → REP → NOT → PUR → PUB. نفّذ **T2.5** = ORG-07 إثبات تحويل الاشتراك ومراجعته (5: ready/validation_error/saving/success/server_error) على فرع `phase2/t2.5-org-07` من `origin/main` (بعد دمج #67–#69): رفع صورة إيصال التحويل → «قيد المراجعة» بلا تفعيل تلقائي ولا وعد بتاريخ (§١٦.٢، §١١.٢)، المراجعة يدوية من المشغّل (نقطة نهاية اعتماد/رفض للمنصة — خلف `is_platform_staff` أو أمر إداري)، `server_error` يبقي الملف، الاعتماد يمدّ `TenantSubscription.expires_at` ويغيّر الباقة؛ زرّا «تجديد الاشتراك» في ORG-06/08 يقودان إليه (`/org/subscription/renew`) بدل تعطيلهما. ابنِ على `core/subscription.py` و`C-UPLOAD` (كما `BackupCopy` في SYS-05). الحلقة نفسها لكل شاشة: `frameTextsAll` ← البناء من `@sting/ui-web` فقط بجذر `data-screen`/`data-state` ← Playwright بـ`expectFrame`/`fromFrame` على 390/834/1440 ← commit ← `gh pr create --base main` ← دمج بعد خضرة CI ← التالي؛ ولكل معيار §١٨ جديد ذي أثر مرئي سيناريو في `apps/web/e2e-gate` وصفّ في `docs/evidence/README.md` (ACC-80–82 في T2.17 بمساعد `POST /api/scenario/subscription`). ما ينتظر المالك في §4 لا تفعله بلا مدخلاته.
...
المجاميع بعد T2.4: **pytest 384 · Vitest 348 (+1 todo) · Playwright 969 (323 × 3 مقاسات) · gate 8/8** (Playwright الكامل لم يُشغَّل محلياً في هذه الجلسة — CI هو المرجع؛ البوابة 7/7 محلياً على Django حقيقي).
...
2. **شغّل الفحوص الأربعة في §3** كلٌّ وحده وتأكد من المجاميع (pytest 384 / Vitest 348 (+1 todo) / Playwright 969 / gate 8 — Playwright بـ`--workers=3` وحده؛ تشغيله مع غيره يجوّعه فتسقط اختبارات بمهلات).
3. **اقرأ §1 بالترتيب** (القسم ٣ من الأمر إلزامي كاملاً).
4. **الفرع**: `git checkout -b phase2/t2.5-org-07 origin/main` بعد دمج #67–#69.
5. **نفّذ T2.5** = ORG-07 (5) وفق PLAN.md §٦. ما بقي من المرحلة ١ ينتظر المالك (§4): عتاد الطباعة (ACC-83)، PWA مثبَّت لإثبات ACC-72، أيقونات/مفاتيح/خطوط، وحسم الافتراضات؛ ACC-63/105/109 تُثبت في T2.3/T2.12/T2.17.
