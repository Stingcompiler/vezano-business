...
آخر تحديث: 2026-09-18 · `main` يحوي T0.1–T0.20 وT1.1–T1.40 (PRs #1–#62 مدمجة) · **PR #63 مفتوح** = T1.41 WEB-02 على `phase1/t1.41-web-02` (ادمجه إذا كانت الفحوص الثلاثة خضراء: `gh pr checks 63` ثم `gh pr merge 63 --merge --delete-branch`) · التالي: T1.42 (WEB-03) على فرع `phase1/t1.42-web-03`
...
> 1. اقرأ `docs/HANDOFF.md` كاملاً (ترتيب القراءة في §1، الحالة في §2، الأوامر في §3، الفخاخ في §3، خطوات البدء في §5) ثم `docs/decisions/0005-acc-01-02-frame-conflicts-and-identity.md` §١–§٤٤ (القرارات والافتراضات المسجّلة — الملاحق §٢١ فما بعد لمهام PTY/INV الأخيرة).
> 2. `git fetch --all` ثم تحقق من `gh pr list`: PR #63 (T1.41 WEB-02) مفتوح — إن كانت `gh pr checks 63` خضراء (3 فحوص) ادمجه `gh pr merge 63 --merge --delete-branch`؛ إن كان أحمر أصلح على فرعه أولاً (PRs #1–#62 مدمجة). ثم أنشئ الفرع `git checkout -b phase1/t1.42-web-03 origin/main`. المالك أذن بدمج كل PR بعد خضرة الفحوص الثلاثة منذ T1.9: `gh pr merge N --merge --delete-branch`.
...
> 5. نفّذ **T1.42** = WEB-03 ربط طابعة ويب عبر Web Bluetooth (4 حالات) وفق `docs/PLAN.md` §٣ (الصف T1.42: `09-D5#WEB-03`؛ §١٢.٢–١٢.٣؛ `23-Handoff` G-10؛ معيار ACC-83): BLE/GATT فقط (Classic يُحال لمسار آخر)؛ القدرة تُفحص قبل عرض الزر (R-09)؛ صفحة تجربة عربية (نص متصل ومختلط وأرقام وباركود وكشف طويل)؛ قائمة الطرازات (G-10) بيانات خارجية لا نصاً مضمّناً؛ الاختبار الفعلي على طرازين يُسجَّل في ملف الأدلة ولا يُدّعى قبله. ابنِ على تنقيط الإيصال في POS-11 (T1.18) وناقل الطباعة؛ لا تخترع شاشة غير مرسومة. بالحلقة نفسها: `frameTextsAll` ← البناء من `@sting/ui-web` فقط بجذر `data-screen`/`data-state` ← Playwright بـ`expectFrame`/`fromFrame` على 390/834/1440 ← commit ← `gh pr create --base main` ← دمج بعد خضرة CI ← التالي بترتيب PLAN.md (T1.43 بوابة السيناريوهات وملف الأدلة).
...
المجاميع بعد T1.41: **pytest 371 · Vitest 348 (+1 todo) · Playwright 906 (302 × 3 مقاسات)** (Playwright الكامل لم يُشغَّل محلياً في هذه الجلسة — CI هو المرجع؛ `web-notify.spec.ts` وحده 24/24 محلياً).
...
| 7 | **افتراضات SYS/WEB تنتظر حسمك** في 0005 §٣٧–§٤٤: `stale` بعد ساعة (§٣٧)؛ لا قناة رجوع لقرار المراجعة إلى الجهاز (§٣٨)؛ أيام المساحة تقدير خشن (§٣٩)؛ تفرّد رقم الفاتورة غير مفروض خادمياً (§٣٧)؛ **أيقونات `manifest.webmanifest` 192/512 من ملفات التصميم — التثبيت على Android يحتاجها (§٤٣)**؛ **مفاتيح VAPID في بيئة النشر (`vapid --gen`) ونصوص أزرار WEB-02 غير المرسومة (§٤٤)** | لا يوقف؛ الأيقونات والمفاتيح تُضاف حين تصل |
...
2. **شغّل الفحوص الثلاثة في §3** وتأكد من المجاميع (pytest 371 / Vitest 348 / Playwright 906 — شغّل Playwright وحده وبـ`--workers=3` على هذا الجهاز؛ تشغيله مع pytest/Vitest يجوّعه فتسقط اختبارات بمهلات). فشلٌ هنا يُبلَّغ ولا يُرقَّع.
3. **اقرأ §1 بالترتيب** (القسم ٣ من الأمر إلزامي كاملاً).
4. **أنشئ الفرع**: `git checkout -b phase1/t1.42-web-03` من `origin/main` (بعد دمج #63).
5. **نفّذ T1.42** = WEB-03 (4) وفق PLAN.md §٣ (`09-D5#WEB-03`؛ §١٢.٢–١٢.٣؛ G-10): Web Bluetooth BLE/GATT فقط، القدرة تُفحص قبل الزر، صفحة تجربة عربية، قائمة طرازات خارجية. ثم T1.43 (بوابة السيناريوهات وملف الأدلة).
6. بعدها T1.43 بترتيب PLAN.md: ACC → HOME → CAT → SHIFT → POS → PTY → INV → SYS → WEB → بوابة T1.43. عند أي غموض: توقّف، سجّل في `docs/decisions/000N-*.md`، اسأل.
