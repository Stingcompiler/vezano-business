...
آخر تحديث: 2026-09-18 · `main` يحوي T0.1–T0.20 وT1.1–T1.34 (PRs #1–#55 مدمجة) · **PR #56 (T1.35: SYS-03، فرع `phase1/t1.35-sys-03`) مفتوح — يُدمج بعد خضرة CI إن لم يكن دُمج** · التالي: T1.36
...
> 1. اقرأ `docs/HANDOFF.md` كاملاً (ترتيب القراءة في §1، الحالة في §2، الأوامر في §3، الفخاخ في §3، خطوات البدء في §5) ثم `docs/decisions/0005-acc-01-02-frame-conflicts-and-identity.md` §١–§٣٨ (القرارات والافتراضات المسجّلة — الملاحق §٢١ فما بعد لمهام PTY/INV الأخيرة).
> 2. `git fetch --all` ثم تحقق من `gh pr list`: إن كان PR #56 (T1.35: SYS-03، فرع `phase1/t1.35-sys-03`) ما زال مفتوحاً وCI أخضر (`gh pr checks 56`) فادمجه بـ`gh pr merge 56 --merge --delete-branch` (المالك أذن بالدمج بعد خضرة CI منذ T1.9)؛ ثم `git checkout -b phase1/t1.36-sys-04-11 origin/main`.
...
> 4. تحقق: `pnpm check` (Vitest 345 + 1 todo) · `cd backend && DATABASE_URL=postgresql:///sting_t115 STING_ENV=test STING_FAULTS_ENABLED=1 uv run pytest -p no:warnings` (362؛ `sting_t115` قاعدة اختبار منفصلة عن `sting_dev` — أنشئها بـ`createdb sting_t115` وطبّق `migrate` عليها إن لم توجد) · `cd apps/web && pnpm exec playwright test --workers=3` (807 = 269 × 3؛ في الخلفية؛ وحده لا مع غيره — يستغرق نحو 25 دقيقة). فشلٌ يُبلَّغ ولا يُرقَّع؛ اختبار يسقط في التشغيل الكامل ويمرّ وحده = حِمل لا عيب (يُذكر في PR).
> 5. نفّذ **T1.36** = SYS-04 انخفاض التخزين أو فشل استدامته (3 حالات) + SYS-11 الدعم والتشخيص (4 حالات) وفق `docs/PLAN.md` §٣ (الصف T1.36: `16-D11#SYS-04`، `19-D14#SYS-11`؛ §١٣.١، §١٣.٦؛ معايير ACC-69، 70، 87): تنبيه عند اقتراب التخزين أو معلّق > 6 ساعات؛ لا حذف لغير المغطّى؛ إصدار التطبيق ومعرّف الجهاز وآخر فشل طباعة بلا أسرار؛ سجل التشخيص لا يحوي رموزاً ولا بيانات مستأجر آخر. ابنِ على `navigator.storage.estimate()`/طلب التخزين المستديم (ACC-05، `@sting/platform`)، وعلى `sync.attempts`/`sync.last_ok`/`sync.halted` (T1.34)، وعلى `printer_fail` (POS-08) و`downloadLocalBackup` (SYS-05)؛ لا تخترع شاشة غير مرسومة. بالحلقة نفسها: `frameTextsAll` ← البناء من `@sting/ui-web` فقط بجذر `data-screen`/`data-state` ← Playwright بـ`expectFrame`/`fromFrame` على 390/834/1440 ← commit ← `gh pr create --base main` ← دمج بعد خضرة CI ← التالي بترتيب PLAN.md.
...
المجاميع بعد T1.35: **pytest 362 · Vitest 345 (+1 todo) · Playwright 807 (269 × 3 مقاسات)**.
...
1. **تحقق من الحالة**: `git status` نظيف؛ `gh pr list` يطابق الجدول في §2 — PR #56 إن كان مفتوحاً وأخضر فادمجه. إن اختلف الوضع، أبلغ المالك قبل المتابعة.
2. **شغّل الفحوص الثلاثة في §3** وتأكد من المجاميع (pytest 362 / Vitest 345 / Playwright 807 — شغّل Playwright وحده وبـ`--workers=3` على هذا الجهاز؛ تشغيله مع pytest/Vitest يجوّعه فتسقط اختبارات بمهلات). فشلٌ هنا يُبلَّغ ولا يُرقَّع.
3. **اقرأ §1 بالترتيب** (القسم ٣ من الأمر إلزامي كاملاً).
4. **أنشئ الفرع**: `git checkout -b phase1/t1.36-sys-04-11` من `origin/main`.
5. **نفّذ T1.36** = SYS-04 (3) + SYS-11 (4) وفق PLAN.md §٣ (`16-D11#SYS-04`، `19-D14#SYS-11`؛ §١٣.١، §١٣.٦): التخزين واستدامته، والدعم والتشخيص بلا أسرار. ثم T1.37 بترتيب PLAN.md.
6. بعدها T1.37 … بترتيب PLAN.md: ACC → HOME → CAT → SHIFT → POS → PTY → INV → SYS → WEB → بوابة T1.43. عند أي غموض: توقّف، سجّل في `docs/decisions/000N-*.md`، اسأل.
