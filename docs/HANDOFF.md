...
آخر تحديث: 2026-09-19 · `main` يحوي T0.1–T0.20 وT1.1–T1.43 وT2.1–T2.2 (PRs #1–#67 مدمجة) · **المرحلة ٢ جارية** (PLAN.md §٦) · PRs مكدّسة: **#68** T2.3 → **#69** T2.4 → **#70** T2.5 ORG-07 (كل فرع فوق سابقه) — ادمجها بالترتيب بعد خضرة الفحوص الأربعة · التالي: T2.6 (ORG-09 إعدادات المنشأة + ORG-10 سجل التدقيق) على `phase2/t2.6-org-09-10`
...
> 1. اقرأ `docs/HANDOFF.md` كاملاً (ترتيب القراءة في §1، الحالة في §2، الأوامر والفخاخ في §3، ما ينتظر المالك في §4، خطوات البدء في §5) ثم `docs/decisions/0005-acc-01-02-frame-conflicts-and-identity.md` §١–§٥١ (القرارات والافتراضات المسجّلة؛ §٤٦ = بوابة T1.43، §٤٧–§٥١ = ORG-01…08).
> 2. `git fetch --all` ثم `gh pr list`: PRs #68 (T2.3) → #69 (T2.4) → #70 (T2.5) مكدّسة كلٌّ فوق سابقه — لكلٍّ بالترتيب: خضرة الفحوص **الأربعة** (backend، packages، e2e، gate) ⇒ `gh pr merge N --merge --delete-branch` (الإذن قائم منذ T1.9 لكل PR أخضر)؛ أحمر ⇒ أصلح على فرعه أولاً. PRs #1–#67 مدمجة.
...
> 5. **المرحلة ٢ جارية بإذن المالك** — الخطة في `docs/PLAN.md` §٦ (16 مهمة + بوابة T2.17) بالترتيب ORG → REP → NOT → PUR → PUB. نفّذ **T2.6** = ORG-09 إعدادات المنشأة واللغة والقوالب (4: ready/validation_error/success/conflict؛ `13-D8#ORG-09`) + ORG-10 سجل التدقيق (4: ready/loading/empty/permission_denied؛ `20-D15#ORG-10`) على فرع `phase2/t2.6-org-09-10` من `origin/main` (بعد دمج #68–#70): قالب الإيصال واللغة وطرق الدفع الفعّالة، `conflict` عند تعديل متزامن (§١١.٣)، العملة غير قابلة للتبديل (ACC-27)؛ سجل التدقيق من مصادره القائمة (تعطيل المستخدم `User.deactivated_*`، مراجعة الإثبات `SubscriptionProof.reviewed_*`، سحب/محو الجهاز، قرارات SYS-03، الدعوات) بمرشّحات ونطاق رؤية يتبع نطاق المسؤولية — بلا حمولات سرّية (§١٣.٥). الحلقة نفسها لكل شاشة: `frameTextsAll` ← البناء من `@sting/ui-web` فقط بجذر `data-screen`/`data-state` ← Playwright بـ`expectFrame`/`fromFrame` على 390/834/1440 ← commit ← `gh pr create --base main` ← دمج بعد خضرة CI ← التالي؛ ولكل معيار §١٨ جديد ذي أثر مرئي سيناريو في `apps/web/e2e-gate` وصفّ في `docs/evidence/README.md`. ما ينتظر المالك في §4 لا تفعله بلا مدخلاته.
...
المجاميع بعد T2.5: **pytest 386 · Vitest 348 (+1 todo) · Playwright 975 (325 × 3 مقاسات) · gate 8/8** (Playwright الكامل لم يُشغَّل محلياً في هذه الجلسة — CI هو المرجع؛ البوابة 7/7 محلياً على Django حقيقي).
...
2. **شغّل الفحوص الأربعة في §3** كلٌّ وحده وتأكد من المجاميع (pytest 386 / Vitest 348 (+1 todo) / Playwright 975 / gate 8 — Playwright بـ`--workers=3` وحده؛ تشغيله مع غيره يجوّعه فتسقط اختبارات بمهلات).
3. **اقرأ §1 بالترتيب** (القسم ٣ من الأمر إلزامي كاملاً).
4. **الفرع**: `git checkout -b phase2/t2.6-org-09-10 origin/main` بعد دمج #68–#70.
5. **نفّذ T2.6** = ORG-09 (4) + ORG-10 (4) وفق PLAN.md §٦. ما بقي من المرحلة ١ ينتظر المالك (§4): عتاد الطباعة (ACC-83)، PWA مثبَّت لإثبات ACC-72، أيقونات/مفاتيح/خطوط، وحسم الافتراضات؛ ACC-63/105/109 تُثبت في T2.3/T2.12/T2.17.
6. بعدها T2.3 … T2.17 بترتيب PLAN.md §٦. لكل شاشة الحلقة نفسها، ولكل معيار مرئي جديد سيناريو في `e2e-gate` وصفّ في ملف الأدلة. عند أي غموض: توقّف، سجّل في `docs/decisions/000N-*.md`، اسأل.
