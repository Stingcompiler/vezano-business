...
آخر تحديث: 2026-09-17 · `main` يحوي T0.1–T0.20 وT1.1–T1.22 (PRs #1–#43 مدمجة) · **PR #44 (T1.23: PTY-03/04، فرع `phase1/t1.23-pty-03-04`) مفتوح — يُدمج بعد خضرة CI إن لم يكن دُمج** · التالي: T1.24
...
> 2. `git fetch --all` ثم تحقق من `gh pr list`: إن كان PR #44 (T1.23: PTY-03/04، فرع `phase1/t1.23-pty-03-04`) ما زال مفتوحاً وCI أخضر (`gh pr checks 44`) فادمجه بـ`gh pr merge 44 --merge --delete-branch` (المالك أذن بالدمج بعد خضرة CI منذ T1.9)؛ ثم `git checkout -b phase1/t1.24-pty-05 origin/main`.
...
> 5. نفّذ **T1.24** = PTY-05 كشف الحساب (7 حالات) وفق `docs/PLAN.md` §٣ (الصف T1.24: `04-D2#PTY-05`؛ §٧.٦، §٨.٨، §١٤.١؛ القاعدة 5 من الأمر؛ معايير ACC-02، 03، 44، 46، 96): كشف متتابع مدين/دائن/رصيد يبدأ بسطر «رصيد افتتاحي» (`parties.OpeningBalance` — T1.23)، البند المعلّق من هذا الجهاز يظهر ويُوسَم «معلّق — هذا الجهاز»، الرصيد المركّب «خادمي X + معلّق هذا الجهاز Y = Z» (`readPendingCreditByParty` + `balance_as_of`)، آخر سداد و«منذ» و«لا يوجد سداد مسجل» حرفاً (السدادات مع T1.25 — حتى ذلك الحين لا سداد)، لا «عمر دين» (G-15)؛ خادمياً `GET /api/parties/{id}/statement` بسطور من المبيعات والمرتجعات والافتتاحي بنطاق الدور (الكاشير رصيد وقت البيع فقط)؛ ثم زرّا «كشف حساب العميل/المورد» في PTY-03 و«كشف الحساب» في PTY-01/02 يصلون إليه (`/parties/{id}/statement`). بالحلقة نفسها: `frameTextsAll` ← البناء من `@sting/ui-web` فقط بجذر `data-screen`/`data-state` ← Playwright بـ`expectFrame`/`fromFrame` على 390/834/1440 ← commit ← `gh pr create --base main` ← دمج بعد خضرة CI ← التالي بترتيب PLAN.md.
...
| 3b | `docs/decisions/0005-acc-01-02-frame-conflicts-and-identity.md` | قاعدة تطبيق «بطاقات الحالة» D26، تعارضات D2/D8/D26، نموذج الهوية (Account بعدة عضويات)، وملاحق كل مهمة T1.x (§٥–§٢٦) بافتراضاتها |
...
#44 phase1/t1.23-pty-03-04                → main (مفتوح عند التسليم — **ادمجه أولاً بعد خضرة CI**)
```

**كل ما سبق مدمج في `main` عدا #44.** فرع البداية للمهمة التالية هو `main` بعد دمج #44؛ الفروع القديمة محذوفة. الدمج من المنفّذ مأذون به بعد خضرة CI (منذ T1.9).
...
المجاميع بعد T1.23: **pytest 345 · Vitest 331 (+1 todo) · Playwright 585 (195 × 3 مقاسات)**.
...
1. **تحقق من الحالة**: `git status` نظيف؛ `gh pr list` يطابق الجدول في §2 — PR #44 إن كان مفتوحاً وأخضر فادمجه. إن اختلف الوضع، أبلغ المالك قبل المتابعة.
2. **شغّل الفحوص الثلاثة في §3** وتأكد من المجاميع (pytest 345 / Vitest 331 / Playwright 585 — شغّل Playwright وحده وبـ`--workers=3` على هذا الجهاز؛ تشغيله مع pytest/Vitest يجوّعه فتسقط اختبارات بمهلات). فشلٌ هنا يُبلَّغ ولا يُرقَّع.
3. **اقرأ §1 بالترتيب** (القسم ٣ من الأمر إلزامي كاملاً).
4. **أنشئ الفرع**: `git checkout -b phase1/t1.24-pty-05` من `main`.
5. **نفّذ T1.24** = PTY-05 كشف الحساب (7) وفق PLAN.md §٣ (`04-D2#PTY-05`؛ §٧.٦، §٨.٨، §١٤.١؛ القاعدة 5؛ ACC-02، 03، 44، 46، 96): كشف متتابع يبدأ بالافتتاحي، المعلّق يُوسَم، الرصيد المركّب، آخر سداد و«منذ» و«لا يوجد سداد مسجل»، لا «عمر دين»؛ `GET /api/parties/{id}/statement` بنطاق الدور. ثم T1.25 بترتيب PLAN.md.
6. بعدها T1.25 … بترتيب PLAN.md: ACC → HOME → CAT → SHIFT → POS → PTY → INV → SYS → WEB → بوابة T1.43. عند أي غموض: توقّف، سجّل في `docs/decisions/000N-*.md`، اسأل.
