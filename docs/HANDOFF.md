...
آخر تحديث: 2026-09-16 · آخر فرع: `phase0/t0.20-scenario-seed` · آخر commit: `c5625ec`
...
> اقرأ `docs/HANDOFF.md` ثم نفّذ ما فيه. لا تعد تصميم شيء؛ كل شاشة من إطارها المرسوم. ابدأ T1.1 بعد اجتياز فحوص §5.
...
#21 phase0/t0.20-scenario-seed            → #20   ← الرأس الحالي
```
...
المجاميع: **pytest 222 · Vitest 264 (+1 todo) · Playwright 15**.
...
- **استخراج نصوص إطار**: من `tools/frame-text` — `frameTexts("ACC-01","ready")` و`drawnStates("ACC-01")`.
...
| 3 | **جواب س٤** في `0002`: تعارض ترقيم G-17 — نظام التصميم يقول G-17 = حد الائتمان، وخطة المعالجة تقول G-17 = تشفير النسخة المحلية. التوصية المسجّلة: اتباع ترقيم نظام التصميم + نص المواصفة §13.3 للتشفير + توزيع صريح لردّ المرتجع | **T1.37 (SYS-05/06)** فقط |
| 4 | Expo/Tauri (المرحلة ٤) | لا تبدأ إلا بموافقة كتابية بعد التجربة الميدانية (§١٥.١ مرحلة د) |
...
4. **أنشئ الفرع**: `git checkout -b phase1/t1.1-acc-01-02` من `main` (إن دُمج) أو من `phase0/t0.20-scenario-seed`.
5. **نفّذ T1.1** = ACC-01 (3 حالات، `28-D21`) + ACC-02 (7 حالات، `06-D2`) بحلقة §3. المرجع: §٩.٤ من المواصفة ومعيار §١٨ ACC-119. مسار التحقق محايد: «رمز تحقق» + «تحقق يدوي بواسطة الدعم» (G-02). لا شيء في T1.1 موقوف على قرار.
6. بعدها T1.2 … بترتيب PLAN.md: ACC → HOME → CAT → SHIFT → POS → PTY → INV → SYS → WEB → بوابة T1.43. عند أي غموض: توقّف، سجّل في `docs/decisions/000N-*.md`، اسأل.
