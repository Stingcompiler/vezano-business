...
آخر تحديث: 2026-09-18 · `main` يحوي T0.1–T0.20 وT1.1–T1.33 (PRs #1–#54 مدمجة) · **PR #55 (T1.34: SYS-01/SYS-02، فرع `phase1/t1.34-sys-01-02`) مفتوح — يُدمج بعد خضرة CI إن لم يكن دُمج** · التالي: T1.35
...
> 1. اقرأ `docs/HANDOFF.md` كاملاً (ترتيب القراءة في §1، الحالة في §2، الأوامر في §3، الفخاخ في §3، خطوات البدء في §5) ثم `docs/decisions/0005-acc-01-02-frame-conflicts-and-identity.md` §١–§٣٧ (القرارات والافتراضات المسجّلة — الملاحق §٢١ فما بعد لمهام PTY/INV الأخيرة).
> 2. `git fetch --all` ثم تحقق من `gh pr list`: إن كان PR #55 (T1.34: SYS-01/SYS-02، فرع `phase1/t1.34-sys-01-02`) ما زال مفتوحاً وCI أخضر (`gh pr checks 55`) فادمجه بـ`gh pr merge 55 --merge --delete-branch` (المالك أذن بالدمج بعد خضرة CI منذ T1.9)؛ ثم `git checkout -b phase1/t1.35-sys-03 origin/main`.
...
> 4. تحقق: `pnpm check` (Vitest 345 + 1 todo) · `cd backend && DATABASE_URL=postgresql:///sting_t115 STING_ENV=test STING_FAULTS_ENABLED=1 uv run pytest -p no:warnings` (360؛ `sting_t115` قاعدة اختبار منفصلة عن `sting_dev` — أنشئها بـ`createdb sting_t115` وطبّق `migrate` عليها إن لم توجد) · `cd apps/web && pnpm exec playwright test --workers=3` (798 = 266 × 3؛ في الخلفية؛ وحده لا مع غيره — يستغرق نحو 25 دقيقة). فشلٌ يُبلَّغ ولا يُرقَّع؛ اختبار يسقط في التشغيل الكامل ويمرّ وحده = حِمل لا عيب (يُذكر في PR).
> 5. نفّذ **T1.35** = SYS-03 تعارض وحجر ومراجعة مالك (4 حالات) وفق `docs/PLAN.md` §٣ (الصف T1.35: `07-D3#SYS-03`؛ §٨.٣ (`conflicted`)، §٩.٣؛ R-03 × R-11؛ معايير ACC-16، 32، 49، 63): النسختان معروضتان، قبول/رفض بسبب، الأصل لا يُكتب فوقه، لا سقف زمني يفرضه النظام؛ تعارض حساب واحد يجمّده وحده وتستمر البقية. ابنِ على `QuarantinedOperation` (الخادم: `reason=conflicted/rejected`، `original`، `reviewed_at/by`، `decision`) و`GET /api/sync/status` (عدّاد المحجور/المتعارض للجهاز)، وعلى حالة `conflict` في Dexie وسجل `sync.attempts:<op>` (T1.34)، وزر «مراجعة المالك» المعطّل بسببه في SYS-02 (`operation-client.tsx`) — فعّله؛ لا تخترع شاشة غير مرسومة. بالحلقة نفسها: `frameTextsAll` ← البناء من `@sting/ui-web` فقط بجذر `data-screen`/`data-state` ← Playwright بـ`expectFrame`/`fromFrame` على 390/834/1440 ← commit ← `gh pr create --base main` ← دمج بعد خضرة CI ← التالي بترتيب PLAN.md.
...
المجاميع بعد T1.34: **pytest 360 · Vitest 345 (+1 todo) · Playwright 798 (266 × 3 مقاسات)**.
...
1. **تحقق من الحالة**: `git status` نظيف؛ `gh pr list` يطابق الجدول في §2 — PR #55 إن كان مفتوحاً وأخضر فادمجه. إن اختلف الوضع، أبلغ المالك قبل المتابعة.
2. **شغّل الفحوص الثلاثة في §3** وتأكد من المجاميع (pytest 360 / Vitest 345 / Playwright 798 — شغّل Playwright وحده وبـ`--workers=3` على هذا الجهاز؛ تشغيله مع pytest/Vitest يجوّعه فتسقط اختبارات بمهلات). فشلٌ هنا يُبلَّغ ولا يُرقَّع.
3. **اقرأ §1 بالترتيب** (القسم ٣ من الأمر إلزامي كاملاً).
4. **أنشئ الفرع**: `git checkout -b phase1/t1.35-sys-03` من `origin/main`.
5. **نفّذ T1.35** = SYS-03 (4) وفق PLAN.md §٣ (`07-D3#SYS-03`؛ §٨.٣، §٩.٣): تعارض وحجر ومراجعة مالك بالنسختين، قبول/رفض بسبب، الأصل لا يُكتب فوقه. ثم T1.36 بترتيب PLAN.md.
6. بعدها T1.36 … بترتيب PLAN.md: ACC → HOME → CAT → SHIFT → POS → PTY → INV → SYS → WEB → بوابة T1.43. عند أي غموض: توقّف، سجّل في `docs/decisions/000N-*.md`، اسأل.
