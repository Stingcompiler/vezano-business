...
آخر تحديث: 2026-09-19 · `main` يحوي T0.1–T0.20 وT1.1–T1.43 (PRs #1–#65 مدمجة؛ المرحلة ١ مكتملة) · **المرحلة ٢ جارية** (PLAN.md §٦) · **PR #66** = T2.1 ORG-01/02 و**PR #67** = T2.2 ORG-03/04 (فرعه `phase2/t2.2-org-03-04` مبني فوق فرع T2.1) — ادمجهما بالترتيب بعد خضرة الفحوص الأربعة · التالي: T2.3 (ORG-05 سحب مستخدم أو نطاق أو جهاز) على `phase2/t2.3-org-05`
...
> 1. اقرأ `docs/HANDOFF.md` كاملاً (ترتيب القراءة في §1، الحالة في §2، الأوامر والفخاخ في §3، ما ينتظر المالك في §4، خطوات البدء في §5) ثم `docs/decisions/0005-acc-01-02-frame-conflicts-and-identity.md` §١–§٤٨ (القرارات والافتراضات المسجّلة؛ §٤٦ = بوابة T1.43، §٤٧ = ORG-01/02، §٤٨ = ORG-03/04).
> 2. `git fetch --all` ثم `gh pr list`: PR #66 (T2.1) وPR #67 (T2.2، مبني فوق فرع #66) — لكلٍّ بالترتيب: خضرة الفحوص **الأربعة** (backend، packages، e2e، gate) ⇒ `gh pr merge N --merge --delete-branch` (الإذن قائم منذ T1.9 لكل PR أخضر)؛ أحمر ⇒ أصلح على فرعه أولاً. PRs #1–#65 مدمجة.
...
> 5. **المرحلة ٢ جارية بإذن المالك** — الخطة في `docs/PLAN.md` §٦ (16 مهمة + بوابة T2.17) بالترتيب ORG → REP → NOT → PUR → PUB. نفّذ **T2.3** = ORG-05 سحب مستخدم أو نطاق أو جهاز (5 حالات: ready/validation_error/permission_denied/pending_sync/success؛ F08) على فرع `phase2/t2.3-org-05` من `origin/main` (بعد دمج #66 و#67): تعطيل فوري للتخويل الجديد؛ الجهاز الذي يحمل معلّقاً يُسترد إلى الحجر قبل المحو (يكمل SYS-07 من طرف المالك — `sync/recovery.py` freeze/wipe موجودان)؛ لا محو لعمليات الآخرين على جهاز مشترك (ACC-45، 63، 64)؛ وأضف سيناريو بوابة ACC-63 في `apps/web/e2e-gate` وصفّه في `docs/evidence/README.md`. ابنِ على `core/org.py` و`core/org_branches.py` (`_device_rows` يعرض المعلّق المبلَّغ) وشاشات `/org/*`. الحلقة نفسها لكل شاشة: `frameTextsAll` ← البناء من `@sting/ui-web` فقط بجذر `data-screen`/`data-state` ← Playwright بـ`expectFrame`/`fromFrame` على 390/834/1440 ← commit ← `gh pr create --base main` ← دمج بعد خضرة CI ← التالي. ما ينتظر المالك في §4 لا تفعله بلا مدخلاته.
...
المجاميع بعد T2.2: **pytest 378 · Vitest 348 (+1 todo) · Playwright 945 (315 × 3 مقاسات) · gate 7/7** (Playwright الكامل لم يُشغَّل محلياً في هذه الجلسة — CI هو المرجع؛ البوابة 7/7 محلياً على Django حقيقي).
...
2. **شغّل الفحوص الأربعة في §3** كلٌّ وحده وتأكد من المجاميع (pytest 378 / Vitest 348 (+1 todo) / Playwright 945 / gate 7 — Playwright بـ`--workers=3` وحده؛ تشغيله مع غيره يجوّعه فتسقط اختبارات بمهلات).
3. **اقرأ §1 بالترتيب** (القسم ٣ من الأمر إلزامي كاملاً).
4. **الفرع**: `git checkout -b phase2/t2.3-org-05 origin/main` بعد دمج #66 و#67.
5. **نفّذ T2.3** = ORG-05 (5) وفق PLAN.md §٦ + سيناريو بوابة ACC-63. ما بقي من المرحلة ١ ينتظر المالك (§4): عتاد الطباعة (ACC-83)، PWA مثبَّت لإثبات ACC-72، أيقونات/مفاتيح/خطوط، وحسم الافتراضات؛ ACC-63/105/109 تُثبت في T2.3/T2.12/T2.17.
