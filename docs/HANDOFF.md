...
آخر تحديث: 2026-09-19 · `main` يحوي T0.1–T0.20 وT1.1–T1.43 وT2.1–T2.9 (PRs #1–#74 مدمجة) · **المرحلة ٢ جارية** (PLAN.md §٦) · PRs مكدّسة: **#75** T2.10 → **#76** T2.11 → **#77** T2.12 NOT-05/06 (كل فرع فوق سابقه) — ادمجها بالترتيب بعد خضرة الفحوص الأربعة · التالي: T2.13 (PUR-01 + PUR-02) على `phase2/t2.13-pur-01-02`
...
> 1. اقرأ `docs/HANDOFF.md` كاملاً (ترتيب القراءة في §1، الحالة في §2، الأوامر والفخاخ في §3، ما ينتظر المالك في §4، خطوات البدء في §5) ثم `docs/decisions/0005-acc-01-02-frame-conflicts-and-identity.md` §١–§٥٨ (القرارات والافتراضات المسجّلة؛ §٤٦ = بوابة T1.43، §٤٧–§٥٢ = ORG-01…10، §٥٣–§٥٥ = REP-01…06، §٥٦–§٥٨ = NOT-01…06).
> 2. `git fetch --all` ثم `gh pr list`: PRs #75 (T2.10) → #76 (T2.11) → #77 (T2.12) مكدّسة كلٌّ فوق سابقه — لكلٍّ بالترتيب: خضرة الفحوص **الأربعة** (backend، packages، e2e، gate) ⇒ `gh pr merge N --merge --delete-branch` (الإذن قائم منذ T1.9 لكل PR أخضر)؛ أحمر ⇒ أصلح على فرعه أولاً. PRs #1–#74 مدمجة.
...
> 5. **المرحلة ٢ جارية بإذن المالك** — الخطة في `docs/PLAN.md` §٦ (16 مهمة + بوابة T2.17) بالترتيب ORG → REP → NOT → PUR → PUB. ORG-01…10 وREP-01…06 وNOT-01…06 مكتملة (T2.1–T2.12). نفّذ **T2.13** = PUR-01 (4) + PUR-02 (4): قائمة أوامر الشراء الداخلية وتفاصيلها وإنشاء أمر (مورّد من الأطراف، كميات بوحدة الشراء ومعاملها، لا تكلفة إلزامية؛ `phase_locked` لما يعتمد على سياسة التكلفة) — الإطارات والمراجع `32-D24#PUR-01/02`؛ §٧.٧، §٣.٣؛ القيد: الأمر لا يحرّك مخزوناً ولا ذمة قبل مستند الشراء — على فرع `phase2/t2.13-pur-01-02` من `origin/main` (بعد دمج #75–#77). ابنِ على `inventory.GoodsReceipt` (INV-04 الاستلام بلا تكلفة إلزامية) والأطراف الموردين (`Party.is_supplier`) وسياسة التكلفة `TenantSettings.inventory.cost_policy` (REP-05) لما هو `phase_locked`. الحلقة نفسها لكل شاشة: `frameTextsAll` ← البناء من `@sting/ui-web` فقط بجذر `data-screen`/`data-state` ← Playwright بـ`expectFrame`/`fromFrame` على 390/834/1440 ← commit ← `gh pr create --base main` ← دمج بعد خضرة CI ← التالي؛ ولكل معيار §١٨ جديد ذي أثر مرئي سيناريو في `apps/web/e2e-gate` وصفّ في `docs/evidence/README.md`. ما ينتظر المالك في §4 لا تفعله بلا مدخلاته.
...
المجاميع بعد T2.12: **pytest 400 · Vitest 348 (+1 todo) · Playwright 1062 (354 × 3 مقاسات؛ مواصفات T2.7–T2.12 الجديدة 12/12 كلٌّ محلياً — التشغيل الكامل لم يُعَد بعد T2.7، CI هو المرجع) · gate 9/9** (البوابة 9/9 محلياً على Django حقيقي).
...
1. **تحقق من الحالة**: `git status` نظيف؛ `gh pr list` — #75 → #76 → #77 مكدّسة أو ما بقي منها: ادمجها بالترتيب بعد خضرة الفحوص الأربعة. إن اختلف الوضع، أبلغ المالك قبل المتابعة.
2. **شغّل الفحوص الأربعة في §3** كلٌّ وحده وتأكد من المجاميع (pytest 400 / Vitest 348 (+1 todo) / Playwright 1062 / gate 9 — Playwright بـ`--workers=3` وحده؛ تشغيله مع غيره يجوّعه فتسقط اختبارات بمهلات).
3. **اقرأ §1 بالترتيب** (القسم ٣ من الأمر إلزامي كاملاً).
4. **الفرع**: `git checkout -b phase2/t2.13-pur-01-02 origin/main` بعد دمج #75–#77.
5. **نفّذ T2.13** = PUR-01 (4) + PUR-02 (4) وفق PLAN.md §٦. ما بقي من المرحلة ١ ينتظر المالك (§4): عتاد الطباعة (ACC-83)، PWA مثبَّت لإثبات ACC-72، أيقونات/مفاتيح/خطوط، وحسم الافتراضات؛ ACC-63/105/109 تُثبت في T2.3/T2.12/T2.17.
6. بعدها T2.14 … T2.17 بترتيب PLAN.md §٦. لكل شاشة الحلقة نفسها، ولكل معيار مرئي جديد سيناريو في `e2e-gate` وصفّ في ملف الأدلة. عند أي غموض: توقّف، سجّل في `docs/decisions/000N-*.md`، اسأل.
