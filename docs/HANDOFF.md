...
آخر تحديث: 2026-09-18 · `main` يحوي T0.1–T0.20 وT1.1–T1.35 (PRs #1–#56 مدمجة) · **PR #57 (T1.36: SYS-04/SYS-11، فرع `phase1/t1.36-sys-04-11`) مفتوح — يُدمج بعد خضرة CI إن لم يكن دُمج** · التالي: T1.37
...
> 1. اقرأ `docs/HANDOFF.md` كاملاً (ترتيب القراءة في §1، الحالة في §2، الأوامر في §3، الفخاخ في §3، خطوات البدء في §5) ثم `docs/decisions/0005-acc-01-02-frame-conflicts-and-identity.md` §١–§٣٩ (القرارات والافتراضات المسجّلة — الملاحق §٢١ فما بعد لمهام PTY/INV الأخيرة).
> 2. `git fetch --all` ثم تحقق من `gh pr list`: إن كان PR #57 (T1.36: SYS-04/SYS-11، فرع `phase1/t1.36-sys-04-11`) ما زال مفتوحاً وCI أخضر (`gh pr checks 57`) فادمجه بـ`gh pr merge 57 --merge --delete-branch` (المالك أذن بالدمج بعد خضرة CI منذ T1.9)؛ ثم `git checkout -b phase1/t1.37-sys-05-06 origin/main`.
...
> 4. تحقق: `pnpm check` (Vitest 345 + 1 todo) · `cd backend && DATABASE_URL=postgresql:///sting_t115 STING_ENV=test STING_FAULTS_ENABLED=1 uv run pytest -p no:warnings` (363؛ `sting_t115` قاعدة اختبار منفصلة عن `sting_dev` — أنشئها بـ`createdb sting_t115` وطبّق `migrate` عليها إن لم توجد) · `cd apps/web && pnpm exec playwright test --workers=3` (825 = 275 × 3؛ في الخلفية؛ وحده لا مع غيره — يستغرق نحو 25 دقيقة). فشلٌ يُبلَّغ ولا يُرقَّع؛ اختبار يسقط في التشغيل الكامل ويمرّ وحده = حِمل لا عيب (يُذكر في PR).
> 5. نفّذ **T1.37** = SYS-05 تصدير نسخة محلية (5 حالات) + SYS-06 استعادة (6 حالات) وفق `docs/PLAN.md` §٣ (الصف T1.37: `16-D11#SYS-05/06`؛ §١٣.٣؛ معايير ACC-73، 74، 81، 85، 99؛ **0002 س٤** سؤال G-17 المعلّق على المالك): نسخة مشفّرة بتحقق سلامة وإصدار مخطط وربط بالمؤسسة وبلا رموز أجهزة أو متحققات PIN؛ استعادة بمعاينة (المؤسسة، الفترة، العدد، التعارضات) ولا تضاعف الأثر عند التكرار وبهوية جديدة للكتابات الجديدة فقط؛ استعادة مرتين على جهاز بديل = لا تكرار؛ ملف تالف أو مستأجر مختلف يُرفض قبل أي تغيير. ابنِ على `lib/local-backup.ts` (`buildLocalBackup`/`downloadLocalBackup` — JSON بلا تشفير اليوم) و`saveOperation` (المعرّف الثابت = لا مضاعفة) و`sync_epoch`/`device.registration`؛ إن لم يُحسم G-17 فسجّل الافتراض (كلمة حماية + WebCrypto AES-GCM) في 0005 واستمر؛ لا تخترع شاشة غير مرسومة. بالحلقة نفسها: `frameTextsAll` ← البناء من `@sting/ui-web` فقط بجذر `data-screen`/`data-state` ← Playwright بـ`expectFrame`/`fromFrame` على 390/834/1440 ← commit ← `gh pr create --base main` ← دمج بعد خضرة CI ← التالي بترتيب PLAN.md.
...
المجاميع بعد T1.36: **pytest 363 · Vitest 345 (+1 todo) · Playwright 825 (275 × 3 مقاسات)**.
...
1. **تحقق من الحالة**: `git status` نظيف؛ `gh pr list` يطابق الجدول في §2 — PR #57 إن كان مفتوحاً وأخضر فادمجه. إن اختلف الوضع، أبلغ المالك قبل المتابعة.
2. **شغّل الفحوص الثلاثة في §3** وتأكد من المجاميع (pytest 363 / Vitest 345 / Playwright 825 — شغّل Playwright وحده وبـ`--workers=3` على هذا الجهاز؛ تشغيله مع pytest/Vitest يجوّعه فتسقط اختبارات بمهلات). فشلٌ هنا يُبلَّغ ولا يُرقَّع.
3. **اقرأ §1 بالترتيب** (القسم ٣ من الأمر إلزامي كاملاً).
4. **أنشئ الفرع**: `git checkout -b phase1/t1.37-sys-05-06` من `origin/main`.
5. **نفّذ T1.37** = SYS-05 (5) + SYS-06 (6) وفق PLAN.md §٣ (`16-D11#SYS-05/06`؛ §١٣.٣؛ 0002 س٤): تصدير نسخة مشفّرة واستعادة بمعاينة بلا مضاعفة. ثم T1.38 بترتيب PLAN.md.
6. بعدها T1.38 … بترتيب PLAN.md: ACC → HOME → CAT → SHIFT → POS → PTY → INV → SYS → WEB → بوابة T1.43. عند أي غموض: توقّف، سجّل في `docs/decisions/000N-*.md`، اسأل.
