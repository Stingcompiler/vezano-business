...
المجاميع بعد T1.40: **pytest 369 · Vitest 348 (+1 todo) · Playwright 882 (294 × 3 مقاسات)**.
...
| 7 | **افتراضات SYS/WEB تنتظر حسمك** في 0005 §٣٧–§٤٣: `stale` بعد ساعة (§٣٧)؛ لا قناة رجوع لقرار المراجعة إلى الجهاز (§٣٨)؛ أيام المساحة تقدير خشن (§٣٩)؛ تفرّد رقم الفاتورة غير مفروض خادمياً (§٣٧)؛ **أيقونات `manifest.webmanifest` 192/512 من ملفات التصميم — التثبيت على Android يحتاجها (§٤٣)** | لا يوقف؛ الأيقونات تُضاف حين تصل |
...
5. **نفّذ T1.41** = WEB-02 (4) وفق PLAN.md §٣ (`09-D5#WEB-02`؛ §١١.٦، §١١.٨): إذن Web Push بعد شرح وتفاعل، `DeviceEndpoint` أدنى، لا محتوى حساس. ثم T1.42 بترتيب PLAN.md.
6. بعدها T1.42 … بترتيب PLAN.md: ACC → HOME → CAT → SHIFT → POS → PTY → INV → SYS → WEB → بوابة T1.43. عند أي غموض: توقّف، سجّل في `docs/decisions/000N-*.md`، اسأل.
