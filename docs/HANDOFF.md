...
آخر تحديث: 2026-09-17 · `main` يحوي T0.1–T0.20 وT1.1–T1.23 (PRs #1–#44 مدمجة) · **PR #45 (T1.24: PTY-05، فرع `phase1/t1.24-pty-05`) مفتوح — يُدمج بعد خضرة CI إن لم يكن دُمج** · التالي: T1.25
...
> 2. `git fetch --all` ثم تحقق من `gh pr list`: إن كان PR #45 (T1.24: PTY-05، فرع `phase1/t1.24-pty-05`) ما زال مفتوحاً وCI أخضر (`gh pr checks 45`) فادمجه بـ`gh pr merge 45 --merge --delete-branch` (المالك أذن بالدمج بعد خضرة CI منذ T1.9)؛ ثم `git checkout -b phase1/t1.25-pty-06 origin/main`.
...
> 5. نفّذ **T1.25** = PTY-06 تسجيل سداد أو ردّ مبلغ (6 حالات) وفق `docs/PLAN.md` §٣ (الصف T1.25: `33-D25#PTY-06`؛ §٧.١، §٧.٤، §١٤.١؛ معايير ACC-03، 14، 15): بحث ثم مبلغ ثم حفظ، وسيلة الدفع الافتراضية قابلة للتغيير؛ يخفض التراكمي دون توزيع على فواتير؛ التحويل «مسجَّل» ثم «مطابق» منفصلان ومرجع التحويل لا يُستهلك مرتين (`bankReferenceUsed`)؛ نوع PUSH جديد `payment_receipt` (سداد نقدي يدخل درج الوردية `cash_debt_receipts` — SHIFT-02، وسطر «سداد نقدي» في PTY-05 مع `last_payment_at`، والذمّة تنخفض عبر `BALANCE_PROVIDERS`)؛ لقطة 100 + سداد محلي 40 = 60 في الرصيد المركّب (ACC-03 — `readPendingCreditByParty` يجب أن يطرح السداد المعلّق أيضاً). «تسجيل سداد» في PTY-05 معطّل اليوم — فعّله إلى `/parties/{id}/payment`. بالحلقة نفسها: `frameTextsAll` ← البناء من `@sting/ui-web` فقط بجذر `data-screen`/`data-state` ← Playwright بـ`expectFrame`/`fromFrame` على 390/834/1440 ← commit ← `gh pr create --base main` ← دمج بعد خضرة CI ← التالي بترتيب PLAN.md.
...
| 3b | `docs/decisions/0005-acc-01-02-frame-conflicts-and-identity.md` | قاعدة تطبيق «بطاقات الحالة» D26، تعارضات D2/D8/D26، نموذج الهوية (Account بعدة عضويات)، وملاحق كل مهمة T1.x (§٥–§٢٧) بافتراضاتها |
...
#45 phase1/t1.24-pty-05                   → main (مفتوح عند التسليم — **ادمجه أولاً بعد خضرة CI**)
```

**كل ما سبق مدمج في `main` عدا #45.** فرع البداية للمهمة التالية هو `main` بعد دمج #45؛ الفروع القديمة محذوفة. الدمج من المنفّذ مأذون به بعد خضرة CI (منذ T1.9).
...
المجاميع بعد T1.24: **pytest 346 · Vitest 331 (+1 todo) · Playwright 603 (201 × 3 مقاسات)**.
...
1. **تحقق من الحالة**: `git status` نظيف؛ `gh pr list` يطابق الجدول في §2 — PR #45 إن كان مفتوحاً وأخضر فادمجه. إن اختلف الوضع، أبلغ المالك قبل المتابعة.
2. **شغّل الفحوص الثلاثة في §3** وتأكد من المجاميع (pytest 346 / Vitest 331 / Playwright 603 — شغّل Playwright وحده وبـ`--workers=3` على هذا الجهاز؛ تشغيله مع pytest/Vitest يجوّعه فتسقط اختبارات بمهلات). فشلٌ هنا يُبلَّغ ولا يُرقَّع.
3. **اقرأ §1 بالترتيب** (القسم ٣ من الأمر إلزامي كاملاً).
4. **أنشئ الفرع**: `git checkout -b phase1/t1.25-pty-06` من `main`.
5. **نفّذ T1.25** = PTY-06 تسجيل سداد أو ردّ مبلغ (6) وفق PLAN.md §٣ (`33-D25#PTY-06`؛ §٧.١، §٧.٤، §١٤.١؛ ACC-03، 14، 15): بحث ثم مبلغ ثم حفظ، يخفض التراكمي دون توزيع، التحويل «مسجَّل» ثم «مطابق»، لا يُستهلك مرجع مرتين؛ نوع PUSH `payment_receipt` بآثاره على الدرج والذمّة والكشف؛ لقطة 100 + سداد محلي 40 = 60. ثم T1.26 بترتيب PLAN.md.
6. بعدها T1.26 … بترتيب PLAN.md: ACC → HOME → CAT → SHIFT → POS → PTY → INV → SYS → WEB → بوابة T1.43. عند أي غموض: توقّف، سجّل في `docs/decisions/000N-*.md`، اسأل.
