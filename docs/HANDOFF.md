...
آخر تحديث: 2026-09-18 · `main` يحوي T0.1–T0.20 وT1.1–T1.40 (PRs #1–#62 مدمجة) · **PR #63** = T1.41 WEB-02 و**PR #64** = T1.42 WEB-03 (فرعه `phase1/t1.42-web-03` مبني فوق فرع T1.41) — ادمجهما بالترتيب إذا كانت الفحوص الثلاثة خضراء (`gh pr checks N` ثم `gh pr merge N --merge --delete-branch`) · التالي: T1.43 (بوابة السيناريوهات وملف الأدلة) على فرع `phase1/t1.43-gate`
...
> 1. اقرأ `docs/HANDOFF.md` كاملاً (ترتيب القراءة في §1، الحالة في §2، الأوامر في §3، الفخاخ في §3، خطوات البدء في §5) ثم `docs/decisions/0005-acc-01-02-frame-conflicts-and-identity.md` §١–§٤٥ (القرارات والافتراضات المسجّلة — الملاحق §٢١ فما بعد لمهام PTY/INV الأخيرة).
> 2. `git fetch --all` ثم تحقق من `gh pr list`: PR #63 (T1.41 WEB-02) وPR #64 (T1.42 WEB-03، مبني فوق فرع #63) — لكلٍّ بالترتيب إن كانت `gh pr checks N` خضراء (3 فحوص) ادمجه `gh pr merge N --merge --delete-branch`؛ إن كان أحمر أصلح على فرعه أولاً (PRs #1–#62 مدمجة). ثم أنشئ الفرع `git checkout -b phase1/t1.43-gate origin/main`. المالك أذن بدمج كل PR بعد خضرة الفحوص الثلاثة منذ T1.9: `gh pr merge N --merge --delete-branch`.
...
> 4. تحقق: `pnpm check` (Vitest 348 + 1 todo) · `cd backend && DATABASE_URL=postgresql:///sting_t115 STING_ENV=test STING_FAULTS_ENABLED=1 uv run pytest -p no:warnings` (371؛ `sting_t115` قاعدة اختبار منفصلة عن `sting_dev` — أنشئها بـ`createdb sting_t115` وطبّق `migrate` عليها إن لم توجد) · `cd apps/web && pnpm exec playwright test --workers=3` (882 = 294 × 3؛ في الخلفية؛ وحده لا مع غيره — يستغرق نحو 25 دقيقة). فشلٌ يُبلَّغ ولا يُرقَّع؛ اختبار يسقط في التشغيل الكامل ويمرّ وحده = حِمل لا عيب (يُذكر في PR).
> 5. نفّذ **T1.43** = مجموعة سيناريوهات البوابة وملف الأدلة وفق `docs/PLAN.md` §٣ (الصف T1.43؛ القسم ٤ من الأمر — بوابة الخروج؛ §١٥.٤؛ معايير ACC-01…87 ذات الأثر المرئي): Playwright بسياقين (جهازان) + خادم حقيقي: انقطاع وعودة؛ بيع/سداد/مرتجع من جهازين؛ طباعة فاشلة؛ استعادة نسخة مرتين؛ الرصيد 160 من جهازين؛ ملف أدلة لكل معيار (§١٨.٤: الاختبار، commit، البيانات، الجهاز، النتيجة، العيوب المفتوحة) — كل سيناريو في جدول §١٥.٤ يمرّ آلياً ومعه خطوات التجربة اليدوية ونتيجتها المتوقعة. مرجع السلوك `48-Sting-App`. ما لا يُثبت (ACC-83 الطباعة على طرازين) يبقى «غير مُثبت» في `docs/evidence/` ولا يُدّعى.
...
المجاميع بعد T1.42: **pytest 371 · Vitest 348 (+1 todo) · Playwright 921 (307 × 3 مقاسات)** (Playwright الكامل لم يُشغَّل محلياً في هذه الجلسة — CI هو المرجع؛ `web-notify` 24/24 و`web-print` 15/15 محلياً).
...
| 7 | **افتراضات SYS/WEB تنتظر حسمك** في 0005 §٣٧–§٤٥: `stale` بعد ساعة (§٣٧)؛ لا قناة رجوع لقرار المراجعة إلى الجهاز (§٣٨)؛ أيام المساحة تقدير خشن (§٣٩)؛ تفرّد رقم الفاتورة غير مفروض خادمياً (§٣٧)؛ **أيقونات `manifest.webmanifest` 192/512 من ملفات التصميم — التثبيت على Android يحتاجها (§٤٣)**؛ **مفاتيح VAPID في بيئة النشر (`vapid --gen`) ونصوص أزرار WEB-02 غير المرسومة (§٤٤)**؛ **عتاد الطباعة: طرازان BLE على جهازين لإثبات ACC-83 وتعبئة `docs/evidence/printers.md` و`models.json` → `tested` (§٤٥)** | لا يوقف؛ الأيقونات والمفاتيح والعتاد تُضاف حين تصل |
...
2. **شغّل الفحوص الثلاثة في §3** وتأكد من المجاميع (pytest 371 / Vitest 348 / Playwright 921 — شغّل Playwright وحده وبـ`--workers=3` على هذا الجهاز؛ تشغيله مع pytest/Vitest يجوّعه فتسقط اختبارات بمهلات). فشلٌ هنا يُبلَّغ ولا يُرقَّع.
3. **اقرأ §1 بالترتيب** (القسم ٣ من الأمر إلزامي كاملاً).
4. **أنشئ الفرع**: `git checkout -b phase1/t1.43-gate` من `origin/main` (بعد دمج #63 و#64).
5. **نفّذ T1.43** = بوابة السيناريوهات وملف الأدلة وفق PLAN.md §٣ (القسم ٤ من الأمر؛ §١٥.٤؛ §١٨.٤): Playwright بسياقين + خادم، وملف أدلة لكل معيار؛ ما لا يُثبت يبقى «غير مُثبت».
6. T1.43 هي آخر مهام المرحلة ١ في PLAN.md: ACC → HOME → CAT → SHIFT → POS → PTY → INV → SYS → WEB → بوابة T1.43. عند أي غموض: توقّف، سجّل في `docs/decisions/000N-*.md`، اسأل.
