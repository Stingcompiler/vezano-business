...
آخر تحديث: 2026-09-19 · `main` يحوي T0.1–T0.20 وT1.1–T1.43 وT2.1–T2.3 (PRs #1–#68 مدمجة) · **المرحلة ٢ جارية** (PLAN.md §٦) · PRs مكدّسة: **#69** T2.4 → **#70** T2.5 → **#71** T2.6 → **#72** T2.7 REP-01/02 (كل فرع فوق سابقه) — ادمجها بالترتيب بعد خضرة الفحوص الأربعة · التالي: T2.8 (REP-03 تقرير المخزون والحركات + REP-04 تقرير الصندوق والورديات) على `phase2/t2.8-rep-03-04`
...
> 1. اقرأ `docs/HANDOFF.md` كاملاً (ترتيب القراءة في §1، الحالة في §2، الأوامر والفخاخ في §3، ما ينتظر المالك في §4، خطوات البدء في §5) ثم `docs/decisions/0005-acc-01-02-frame-conflicts-and-identity.md` §١–§٥٣ (القرارات والافتراضات المسجّلة؛ §٤٦ = بوابة T1.43، §٤٧–§٥٢ = ORG-01…10، §٥٣ = REP-01/02).
> 2. `git fetch --all` ثم `gh pr list`: PRs #69 (T2.4) → #70 (T2.5) → #71 (T2.6) → #72 (T2.7) مكدّسة كلٌّ فوق سابقه — لكلٍّ بالترتيب: خضرة الفحوص **الأربعة** (backend، packages، e2e، gate) ⇒ `gh pr merge N --merge --delete-branch` (الإذن قائم منذ T1.9 لكل PR أخضر)؛ أحمر ⇒ أصلح على فرعه أولاً. PRs #1–#68 مدمجة.
...
> 5. **المرحلة ٢ جارية بإذن المالك** — الخطة في `docs/PLAN.md` §٦ (16 مهمة + بوابة T2.17) بالترتيب ORG → REP → NOT → PUR → PUB. ORG-01…10 وREP-01/02 مكتملة (T2.1–T2.7). نفّذ **T2.8** = REP-03 تقرير المخزون والحركات (5 حالات؛ `19-D14#REP-03`: بوحدة البيع، لا «إجمالي قطع» R-08) + REP-04 تقرير الصندوق والورديات (5؛ `16-D11#REP-04`: `conflict` = فرق عدّ لم يُراجَع — SHIFT-05) على فرع `phase2/t2.8-rep-03-04` من `origin/main` (بعد دمج #69–#72)؛ §١٠.٣، §٧.٧؛ يطابق INV-01 وSHIFT-02/05. ابنِ على `core/report_views.py` و`sales/reports.py` (نمط `completeness` وسطر الاكتمال والتصدير `?export=csv`) و`features/reports/*`. الحلقة نفسها لكل شاشة: `frameTextsAll` ← البناء من `@sting/ui-web` فقط بجذر `data-screen`/`data-state` ← Playwright بـ`expectFrame`/`fromFrame` على 390/834/1440 ← commit ← `gh pr create --base main` ← دمج بعد خضرة CI ← التالي؛ ولكل معيار §١٨ جديد ذي أثر مرئي سيناريو في `apps/web/e2e-gate` وصفّ في `docs/evidence/README.md`. ما ينتظر المالك في §4 لا تفعله بلا مدخلاته.
...
المجاميع بعد T2.7: **pytest 391 · Vitest 348 (+1 todo) · Playwright 1002 (334 × 3 مقاسات؛ التشغيل الكامل محلياً: 922 ناجح + 80 مهلة تحميل في مواصفات ORG سقطت تحت حمل pytest متزامن ونجحت كلها وحدها 51/51 — CI هو المرجع) · gate 8/8** (البوابة 7/7 محلياً على Django حقيقي).
...
1. **تحقق من الحالة**: `git status` نظيف؛ `gh pr list` — #69 → #70 → #71 → #72 مكدّسة أو ما بقي منها: ادمجها بالترتيب بعد خضرة الفحوص الأربعة. إن اختلف الوضع، أبلغ المالك قبل المتابعة.
2. **شغّل الفحوص الأربعة في §3** كلٌّ وحده وتأكد من المجاميع (pytest 391 / Vitest 348 (+1 todo) / Playwright 1002 / gate 8 — Playwright بـ`--workers=3` وحده؛ تشغيله مع غيره يجوّعه فتسقط اختبارات بمهلات).
3. **اقرأ §1 بالترتيب** (القسم ٣ من الأمر إلزامي كاملاً).
4. **الفرع**: `git checkout -b phase2/t2.8-rep-03-04 origin/main` بعد دمج #69–#72.
5. **نفّذ T2.8** = REP-03 (5) + REP-04 (5) وفق PLAN.md §٦. ما بقي من المرحلة ١ ينتظر المالك (§4): عتاد الطباعة (ACC-83)، PWA مثبَّت لإثبات ACC-72، أيقونات/مفاتيح/خطوط، وحسم الافتراضات؛ ACC-63/105/109 تُثبت في T2.3/T2.12/T2.17.
6. بعدها T2.9 … T2.17 بترتيب PLAN.md §٦. لكل شاشة الحلقة نفسها، ولكل معيار مرئي جديد سيناريو في `e2e-gate` وصفّ في ملف الأدلة. عند أي غموض: توقّف، سجّل في `docs/decisions/000N-*.md`، اسأل.
