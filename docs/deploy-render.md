...
2. املأ الأسرار (`sync: false`). لا يتوقف أي منها الإطلاق؛ الغائب يعطّل قناته وحدها:
   ...
   | `STING_VAPID_PUBLIC_KEY`، `STING_VAPID_PRIVATE_KEY`، `STING_VAPID_SUBJECT` | يُولَّد مرة واحدة: `cd backend && uv run vapid --gen`؛ و`SUBJECT` بصيغة `mailto:` | لا إشعارات ويب |
