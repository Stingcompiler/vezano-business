  ...
  test("loading → ready → expired → empty: أربع حالات بمعناها للمشتري، والمنتهي وحده، ولا عروض", async ({
    ...
    await expectFrame(page, info, {
      ...
      texts: fromFrame("MP-10", "loading", [
        ...
        "الانتهاء هو العمل",
        "عرضٌ منتهٍ يختفي من نتائج المشترين بصمت. العدّاد يجعل الفقد المؤجَّل مرئياً قبل وقوعه",
      ]),
    });
    released = true;
    release();
    await expectFrame(page, info, {
      ...
      texts: fromFrame("MP-10", "ready", [
        ...
        "النشر اختيار صنف صنف مع جمهوره وسعره وصلاحيته",
        "من ينشر ٦٤٠ صنفاً بضغطة يكشف كلفته وتوفّره لمنافسيه بلا أن يقصد.",
        "منشور — 14",
        "مسودة — 5",
        "منتهٍ — 3",
        "مخفي — 2",
    ...
    await expectFrame(page, info, {
      screenId: "MP-10",
      state: "empty",
      texts: fromFrame("MP-10", "empty", ["النشر اختيار صنف صنف مع جمهوره وسعره وصلاحيته"]),
    });
  ...
  test("ready → validation_error → saving → success: صنف يُختار، حقلان ناقصان يمنعان النشر، ثم نُشر بصلاحية تبدأ الآن", async ({
    ...
    await expectFrame(page, info, {
      ...
      texts: fromFrame("MP-11", "ready", [
        ...
        "لا نشر كل المخزون",
        "زرّ «انشر الكتالوج كله» غير موجود عمداً — النشر قرارٌ صنفاً صنفاً بسعرٍ مقصود",
        "والرصيد الداخلي لا يُنشر أبداً.",
      ]),
    });
    ...
    await expectFrame(page, info, {
      ...
      texts: fromFrame("MP-11", "validation_error", [
        ...
        "منشأة ثالثة لا ترى هذا السعر ولا تعرف بوجوده. الرفض عام بلا تسريب.",
        "متابعو منشأتك فقط",
        "المتابعة اشتراك B2B مستقل، وإلغاؤه لا يمحو أحداث الطلبات السابقة",
      ]),
    });
    ...
    await expectFrame(page, info, {
      ...
      texts: fromFrame("MP-11", "success", [
        ...
        "الصلاحية تبدأ الآن",
        "وتنتهي بتاريخها ما لم تُجدَّد صريحاً",
        "النشر ليس التزاماً أبدياً — وانتهاؤه ليس عطلاً.",
      ]),
    });
  });
  ...
  test("permission_denied: ناشر الكتالوج يحرّر المسودة والنشر بصلاحية أعلى", async ({
    ...
    await expectFrame(page, info, {
      ...
      texts: fromFrame("MP-11", "permission_denied", [
        "التحرير غير النشر",
        "ناشر الكتالوج يحرّر المسودة، والنشر النهائي بصلاحية أعلى إن ضُبط كذلك في",
        "فصل مقصود",
        "سعرٌ يخرج للسوق باسم المنشأة التزامٌ تجاري — الفصل بين من يكتب ومن يعتمد قاعدةُ السجل نفسها.",
        "حفظ كمسودة",
      ]),
    });
    await expect(page.locator('[data-screen="MP-11"]')).toContainText(
      "إن ضُبط كذلك في «الأدوار والصلاحيات».",
    );
