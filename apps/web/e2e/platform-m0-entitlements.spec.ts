...
test.describe("PLT-11", () => {
  test("loading → stale → ready: التجميع بختمه الزمني، ثم إعادة التجميع تعرض كل رقم بمقامه وقيمة التجارة مرة واحدة", async ({
    ...
    await expectFrame(page, info, {
      ...
      texts: fromFrame("PLT-11", "stale", [
        ...
        "قيمة التجارة لا تُضاعف بين المشتري والبائع.",
        "نطاق الإصدار الأول لا يُقرأ كنتيجة.",
        "بيانات قديمة",
    ...
    await expectFrame(page, info, {
      ...
      texts: fromFrame("PLT-11", "ready", [
        "مؤشرات M0",
        "الفرص والوساطة والدقائق والكلفة مقابل الأهداف المعلنة. وقيمة التجارة تُحسب مرة لا مرتين للطرفين",
        "لا مضاعفة",
  ...
  test("empty: ثلاث فرص لا تكفي للحكم — نقولها كذلك ولا نعرض نسبة، ولا توقف البناء", async ({
    ...
    await expect(root).toContainText("«غير حاسم — يحتاج 15 فرصة» أصدق من «67% نجاح».");
