  ...
  test("ready → success: ما الذي يُسحب والعدد قبل التأكيد؛ الخيار الأوسط ليس الافتراضي؛ سُحب الوصول", async ({
    ...
    await expectFrame(page, info, {
      ...
      texts: fromFrame("ORG-05", "success", [
        "سُحب الوصول",
        "لا اختفاء صامت",
        "الموظف يرى على جهازه رسالةً تقول إن وصوله سُحب — لا شاشةَ خطأٍ غامضة يظنّها عطباً فيتصل بالدعم (ACC-45).",
        "في سجل التدقيق",
        "باسم من سحب ووقته وسببه إن كُتب. السحب فعلٌ يُراجَع لاحقاً كغيره.",
      ]),
    });
    await expect(root).toContainText("قابلة للاسترداد من SYS-07.");
