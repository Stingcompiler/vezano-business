...
test.describe("INV-05", () => {
  test("ready → saved_local: المتوقَّع محجوب حتى يُدخل المعدود؛ يُحفظ بعد كل صنف ويُستأنف", async ({
    ...
    await expect(root).toContainText("التسوية تحتاج مراجعة واعتماداً في «مراجعة فروق الجرد».");
  ...
  test("partial → saving → success: جرد جزئي مشروع، والإغلاق يوثّق ولا يُسوّي", async ({
    ...
    await expectFrame(page, info, {
      ...
      texts: fromFrame("INV-05", "success", [
        "أُغلقت الجلسة",
        "المعدود والمتوقَّع والفوارق بعددها وقيمتها. والتسوية قرارٌ تالٍ في",
        "لا أثرٌ تلقائي.",
        "الإغلاق لا يُسوّي",
    ...
    await expect(page.locator("body")).toContainText(
      "والتسوية قرارٌ تالٍ في «مراجعة فروق الجرد» لا أثرٌ تلقائي.",
    );
  ...
  test("ready → validation_error → success: سبب لكل فرق، لا سبب عامّ، ثم سُوّيت 4 فروق — مستند TS", async ({
    ...
    await expectFrame(page, info, {
      ...
      texts: fromFrame("INV-06", "success", [
        "سُوّيت",
        "فروق — مستند",
        "الرصيد تغيّر بحركة لها مستند وفاعل وسبب، وتظهر في",
        "كسطر «تسوية جرد» لا كرصيد تغيّر بلا تفسير. العدّ الأصلي محفوظ كما أُدخل ولا يُعاد كتابته.",
      ]),
    });
    await expect(page.locator("body")).toContainText("وتظهر في «حركات المخزون» كسطر «تسوية جرد»");
