...
test.describe("MP-01", () => {
  test("loading → ready → empty: بلا حساب، الموردون بشاراتهم والعروض بوحدتها، ثم منطقة بلا موردين", async ({
    ...
    // 0005 §١٢٧ — بلا أرقام مرجعية داخلية في النصّ المعروض
    const mp = page.locator('[data-screen="MP-01"]');
    await expect(mp).toContainText(
      "حساب السوق مجاني ولا يشترط شراء نقطة البيع — ولا يُحتسب اشتراك إدارة مدفوعاً.",
    );
    await expect(mp).toContainText("ولو فُتح الرابط من هاتف مشترٍ مخوَّل.");
    await expect(mp).not.toContainText(/ACC-\d|§/);
    ...
    await expectFrame(page, info, {
      ...
      texts: fromFrame("MP-01", "empty", [
        ...
        "يعمل بلا سوق: ذمم وطلبات ومستندات بينك وبينهم بلا حاجة إلى وجودهم هنا.",
      ]),
    });
    await expect(page.locator('[data-screen="MP-01"]')).toContainText("دفتر الموردين يعمل بلا سوق");
  });
  ...
  test("stale → offline: عروض من آخر جلب بوقتها حين ينقطع الاتصال، ولا كاش = السوق يحتاج اتصالاً", async ({
    ...
    await expectFrame(fresh, info, {
      ...
      texts: fromFrame("MP-01", "offline", [
        ...
        "نقولها ونحفظ العمل",
        "«السوق يعمل بالاتصال» مع ما يبقى متاحاً: مسودات سلتك",
        "وطلباتك المحفوظة. صفحةٌ بيضاء تُقرأ عطلاً في التطبيق كله.",
      ]),
    });
  });
});
  ...
  test("loading → ready → empty → stale: الدليل بالخدمة، لا منشآت بأزرار تحمل أثرها، والمتقادم بوقته", async ({
    ...
    await expectFrame(page, info, {
      ...
      texts: fromFrame("MP-02", "stale", [
        ...
        "عدد العروض ليس سعراً مؤكَّداً — التأكيد في",
        "مخزن البركة للجملة — تجريبي",
      ]),
    });
    await expect(page.locator('[data-screen="MP-02"]')).toContainText(
      "التأكيد في صفحة العرض وحدها.",
    );
    await context.setOffline(false);
  });
});
