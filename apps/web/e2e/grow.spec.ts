  ...
  test("ready: رأيٌ يُعرَض بسببه — لا صندوق محدَّد سلفاً؛ التحويل باختيار صريح إلى PUR-02", async ({
    ...
    await expectFrame(page, info, {
      ...
      texts: fromFrame("GROW-01", "ready", [
        ...
        "المقترح",
        "على أيّ أساس",
        "لا صندوق اختيار محدَّداً سلفاً ولا «اختر الكل» بارزاً. الاقتراح يصير أمر شراء (PUR-02) باختيار صريح، والكميات تبقى قابلة للتعديل قبل ذلك وبعده.",
    ...
    await expect(page.getByRole("button", { name: "افتح الأمر (PUR-02)" })).toBeVisible();
  });
  ...
  test("permission_denied: أمين المخزن يرى ولا يحوّل — يرسل الاقتراح إلى ندى بملاحظاته", async ({
    ...
    await expectFrame(page, info, {
      ...
      texts: fromFrame("GROW-01", "permission_denied", [
        ...
        "العمود المحجوب",
        "القيمة التقديرية محجوبة كما في PUR-01 — وللسبب نفسه.",
