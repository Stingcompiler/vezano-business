  ...
  test("validation_error → ready: قسم تحت حدّه الأدنى بالفارق، ثم مستوفى — ولا مجموع كلّي", async ({
    ...
    await expectFrame(page, info, {
      ...
      texts: fromFrame("ORD-01", "validation_error", [
        ...
        "كرتونة 12 عند مورد و24 عند آخر لا تُقارَن بالسعر",
        "نعرض السعر للوحدة الأساسية ونسمّي الفرق.",
    ...
    await expectFrame(page, info, {
      ...
      texts: fromFrame("ORD-01", "ready", [
        ...
        "طلبان لا طلب",
        "الإرسال يُنشئ طلباً لكل مورد",
        "طلبٌ واحد لموردين يعني مسؤوليةً مشتركة لا تقبلها منشأتان مستقلتان.",
        "لا مجموع كلّي",
  ...
  test("stale: السعر تغيّر بعد المسودة — السعران والفرق وقبول سطراً سطراً، ولا تأكيد محلي", async ({
    ...
    await expectFrame(page, info, {
      ...
      texts: fromFrame("ORD-01", "stale", [
        ...
        "لا تأكيد محلي",
        "المسودة لا تُثبّت سعراً",
        "السعر يُثبت بقبول المورد لا بحفظ المشتري.",
        "الموافقة صريحة",
  ...
  test("offline → empty: بلا اتصال تُحفظ المسودة ولا رقم طلب، ثم سلة فارغة بمسارها", async ({
    ...
    await expectFrame(page, info, {
      ...
      texts: fromFrame("ORD-01", "offline", [
        ...
        "المسودة تُحفظ محلياً، والإرسال يحتاج شبكة، وتغيّر السعر يحتاج موافقتك بعد الاتصال.",
        "بلا اتصال",
    ...
    await expectFrame(page, info, {
      ...
      texts: fromFrame("ORD-01", "empty", [
        ...
        "«ابحث في السوق»",
        "«موردون تتابعهم»",
        "ولا اقتراح إعادة توريد: ذاك",
      ]),
    });
    await expect(page.locator('[data-screen="ORD-01"]')).toContainText(
      "ذاك مؤجَّل إلى مرحلة النموّ.",
    );
  });
});
  ...
  test("validation_error: عملة العرض تخالف عملة حسابك — لا تحويل ضمني ولا إرسال (ACC-140)", async ({
    ...
    await expectFrame(page, info, {
      ...
      texts: fromFrame("ORD-02", "validation_error", [
        ...
        "لا تحويل ضمني",
        "نمنع التأكيد ولا نحوّل بسعر صرف مفترض",
        "سعر الصرف التزامٌ مالي لا تقديرٌ في واجهة.",
      ]),
    });
  ...
  test("server_error: الشبكة قُطعت أثناء الإرسال — إعادة المحاولة بالمعرّف نفسه، والاستعلام يجد الطلب", async ({
    ...
    await expectFrame(page, info, {
      ...
      texts: fromFrame("ORD-02", "server_error", [
        ...
        "المعرّف محفوظ",
        "إعادة المحاولة تُرسل الطلب نفسه بمعرّفه لا طلباً جديداً",
        "ومسار «استعلم عن الحالة» معروض",
      ]),
    });
