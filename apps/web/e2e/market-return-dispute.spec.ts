  ...
  test("ready → validation_error → success → partial: القابل للإرجاع، الفحص على الخادم لا على الشاشة، ولا خصم قبل التنفيذ", async ({
    ...
    await expectFrame(page, info, {
      ...
      texts: fromFrame("ORD-11", "ready", [
        ...
        "المرتجع ثلاث خطوات لا خطوة: طلب، موافقة مورد، ثم تنفيذ مادّي بمستند عكسي. لا خصم من الذمّة قبل التنفيذ، ولا تجاوز للمستلَم غير المُعاد",
        "مرتجع على الطلب",
        "ORD-7741",
  ...
  test("partial → success: الخلاف بدفترين ورقم المورد كما هو، الدليل يقلب الدور، والإغلاق بمرجع لا يحرّك دفتراً", async ({
    ...
    await expectFrame(page, info, {
      ...
      texts: fromFrame("ORD-12", "partial", [
        ...
        "إغلاق تذكرة الدعم لا يسوّي دفتراً. ما يُغلق هو التذكرة، وما يسوّي هو إجراء مخوَّل من الطرف نفسه.",
        "جزئي",
        "خلاف",
        "DSP-311",
        "— فارق كرتونة واحدة",
    ...
    await page.getByRole("button", { name: "طلب وسيط من فيزانو" }).click();
    await expect(page.getByRole("button", { name: "طلب وسيط من فيزانو" })).toHaveAttribute(
      "aria-disabled",
      "true",
    );
    ...
    await expectFrame(page, info, {
      ...
      texts: fromFrame("ORD-12", "success", [
        ...
        "ما يُسوّى يُسوّى بمستند مستقل يُرى في كشف الطرف",
        "إغلاقُ تذكرةٍ ليس قيداً.",
      ]),
    });
  ...
  test("ready → permission_denied: من عليه الدور معلَن، ومسؤول الاستلام يرفع الدليل ولا يسوّي", async ({
    ...
    await expectFrame(page, info, {
      ...
      texts: fromFrame("ORD-12", "permission_denied", [
        ...
        "حدّ الدور",
        "التسوية إقرارٌ مالي يخضع لحدّ",
        "والشهادة على ما رآه ليست تنازلاً عن مال.",
      ]),
    });
    await expect(page.locator('[data-screen="ORD-12"]')).toContainText(
      "يخضع لحدّ الدور في «الأدوار والصلاحيات».",
    );
  });
