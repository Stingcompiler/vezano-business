  ...
  test("validation_error: افتتاحي على صنف له حركات — المخرج تسوية جرد", async ({ page }, info) => {
    ...
    await expectFrame(page, info, {
      ...
      texts: fromFrame("INV-03", "validation_error", [
        ...
        "كما PTY-04: الافتتاحي مرة واحدة وقبل أول حركة.",
        "المخرج",
        "تسوية جرد (INV-06) بسبب مكتوب — وهي الطريق الصحيح لتصحيح رصيد له تاريخ.",
        "سكر أبيض",
      ]),
    });
  });
  ...
  test("ready → saving → success: 40 كرتونة 12×1كغ = 480 كيس بلا تكلفة، ثم «استلام آخر»", async ({
    ...
    await expectFrame(page, info, {
      ...
      texts: fromFrame("INV-04", "ready", [
        ...
        "لا نفرض وحدة تكلفة.",
        "من يستلم كرتونة ولا يعرف سعر الحبّة يُدخل الكمية وحدها، والاستلام يكتمل. حساب تكلفة الوحدة يبقى معروضاً كأمر اختياري في PUR-05 — لا سدّاً يمنع دخول البضاعة.",
      ]),
      styles: [[".cat-head__title", "color", "brand.strong"]],
    });
