  ...
  test("permission_denied: المدير لا يدعو باسم المنشأة — الدعوة تفتح باباً ولا تدخل أحداً منه", async ({
    ...
    await expectFrame(page, info, {
      ...
      texts: fromFrame("MP-07", "permission_denied", [
        ...
        "دعوة منشأة أخرى إلى علاقة تجارية فعلٌ باسم منشأتك.",
        "لا نشر تلقائياً",
        "قبول المدعوّ لا ينشر ملفه ولا كتالوجه (ACC-06 · ACC-118) — الدعوة تفتح باباً ولا تدخل أحداً منه.",
      ]),
    });
  ...
  test("expired: رابط دعوة انقضى — جديدٌ لا إحياء؛ والرابط الساري يُفتح بلا حساب بمعاينة عامة", async ({
    ...
    await expectFrame(page, info, {
      ...
      texts: fromFrame("MP-07", "expired", [
        ...
        "جديدٌ لا إحياء",
        "يُنشأ رابط جديد بعمر جديد؛ القديم لا يُمدَّد — تمديد رابطٍ خرج يفتح باباً لا يُعرف من وراءه (ACC-06).",
      ]),
    });
  ...
  test("permission_denied: منشأة معلَّقة — يُمنع الجديد ويبقى السابق، والزر موقوف بنصّ (ACC-135)", async ({
    ...
    await expectFrame(page, info, {
      ...
      texts: fromFrame("MP-14", "permission_denied", [
        ...
        "تعليق البائع يمنع طلباً جديداً ويُبقي الطلبات القائمة والتصدير. الرقم المنتهي يُعرض كـ«آخر سعر معروف» لا كسعر (ACC-135).",
        "معلّق",
  ...
  test("expired → ready: عرض منتهٍ في سلّة مشترٍ بآخر سعر معروف لا يُحذف ولا يُحدَّث صامتاً، ثم لا مانع", async ({
    ...
    await expectFrame(page, info, {
      ...
      texts: fromFrame("MP-14", "ready", [
        ...
        "وجهان لحقيقة واحدة",
        "الناشر يحتاج رؤية المنتهي ليجدّده؛ والمشتري لا يحتاج سعراً ميتاً. الشاشة نفسها بجمهورين (ACC-110 · ACC-143).",
        "المعلَّقة أوسع",
        "تعليق المنشأة يمنع الجديد كله ويبقي السابق للاطلاع والتصدير — لا محو (ACC-135).",
      ]),
    });
  ...
  test("ready → validation_error → success: انتحال بلا دليل لا يُرسل، ثم بلاغ برقم متابعة لا يُعلِّق شيئاً", async ({
    ...
    await expectFrame(page, info, {
      ...
      texts: fromFrame("MP-15", "ready", [
        ...
        "التعليق قرار مراجِع بشري في PLT-07. لو كان البلاغ يُخفي عرضاً تلقائياً لصار سلاحاً بيد منافس — عشرة بلاغات كاذبة تُخرج بائعاً من السوق بلا أن يراجعها أحد.",
      ]),
    });
  ...
  test("permission_denied: رابط متابعة بلاغ فُتح من غير مقدِّمه — هوية المبلّغ محفوظة", async ({
    ...
    await expectFrame(page, info, {
      ...
      texts: fromFrame("MP-15", "permission_denied", [
        ...
        "هوية المبلّغ محفوظة",
        "المبلَّغ عنه لا يرى من بلّغ، وغير المقدِّم لا يرى البلاغ أصلاً. كشفُ المبلّغين يقتل الإبلاغ في سوقٍ صغيرة يعرف الجميع فيها الجميع (PLT-07).",
      ]),
    });
