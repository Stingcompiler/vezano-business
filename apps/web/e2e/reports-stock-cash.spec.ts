  ...
  test("loading → ready → conflict: الفوارق في الأعلى، ووردية بعدّين خارج المجموع ويُقال ذلك", async ({
    ...
    await expectFrame(page, info, {
      ...
      texts: fromFrame("REP-04", "conflict", [
        ...
        "لا نجمع ولا نرجّح",
        "لا نأخذ المتوسط ولا الأحدث. تُعرض النسختان باسم من عدّ ووقته، وتُحال إلى",
        "الصفّ يُستثنى من المجموع",
        "«وردية واحدة خارج المجموع — بانتظار الحسم»",
      ]),
    });
    await expect(page.locator("body")).toContainText("وتُحال إلى «مراجعة التعارضات».");
    await expect(root).toContainText(
    ...
    await expect(page.getByRole("button", { name: "الحسم في مراجعة التعارضات" })).toBeVisible();
  });
