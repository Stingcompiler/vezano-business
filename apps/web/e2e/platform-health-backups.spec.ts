  ...
  test("loading → ready → stale: مؤشرات حيّة بلا محتوى، والقياس المتعذّر يُعلَن متقادماً لا «سليماً»", async ({
    ...
    await expectFrame(page, info, {
      ...
      texts: fromFrame("PLT-09", "ready", [
        ...
        "محدَّث",
        "تشخيص لا محتوى.",
        "كل ما هنا عدّادات وأطوار وأزمنة استجابة — لا معرّف معاملة ولا مبلغ ولا اسم زبون. حين يتعثّر مستأجر بعينه يظهر عدد أجهزته المتأخرة فقط، والدخول إلى دفتره يبقى محكوماً بتذكرة وإذن مؤقت",
      ]),
    });
  ...
  test("server_error: عقدة متعثّرة — تُعرض بنطاقها ووقت آخر نجاح والطابور المعلَّق، بلا ردّ جيل قديم", async ({
    ...
    await expectFrame(page, info, {
      ...
      texts: fromFrame("PLT-09", "server_error", [
        "عند تعثُّر عقدة خادمية: تُعرض العقدة المتأثرة ونطاق المستأجرين عليها ووقت آخر نجاح — ولا يُطبَّق ردّ جيل قديم فوق ما نجح على أجهزة التجار. الأثر يُصف طابوراً معلَّقاً يحسمه مسار مخوَّل",
        "آخر نجاح لكل عقدة خادمية",
      ]),
    });
    const root = page.locator('[data-screen="PLT-09"]');
  ...
  test("loading → ready → success: RPO/RTO من آخر تجربة فعلية، وتجربة معزولة تُسجَّل بمن نفّذها", async ({
    ...
    await expectFrame(page, info, {
      ...
      texts: fromFrame("PLT-10", "loading", [
        ...
        "RPO وRTO قياسان",
        "يُعرضان كنتيجة مقاسة لا كوعد",
        "النسخة التي لم تُستعَد تجريبياً مجهولة الصلاحية.",
      ]),
    });
    released = true;
    await expectFrame(page, info, {
      ...
      texts: fromFrame("PLT-10", "ready", [
        ...
        "النسخ تُختبر باستعادة فعلية دورية. الأرقام معروضة كقياس محقَّق، والاستعادة فوق بيانات حيّة محكومة بمسار متعدّد الموافقات",
        "تجربة ناجحة",
