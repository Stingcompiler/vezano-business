...
const SCREENS: { id: string; path: string; texts: string[]; alt: string }[] = [
  ...
  },
  {
    ...
    texts: [
      ...
      "البديل الحاضر",
      "تقرير الأصناف تحت حدّ الأمان موجود اليوم",
      "نحيل إليه بدل أن نترك المستخدم أمام باب مغلق.",
      "اقتراح التوريد — 9 أصناف",
      "لا شيء يُطلب حتى تختار",
    ],
    alt: "الأصناف تحت حدّ الأمان",
  },
  {
    id: "GROW-02",
    path: "/grow/suppliers",
    texts: [
      ...
      "آخر خمسة مستندات",
      "قفل مرحلة. والبديل الحاضر اليوم هو كشف حساب المورد وسجل مستنداته في",
    ],
    alt: "كشف حساب المورد",
  },
  {
  ...
  test(`${s.id} phase_locked: المحتوى مرئي ومعطَّل خلف الوسم، والبديل الحاضر معلَن`, async ({
    ...
    await expect(
      page.getByRole("button", {
        name: `يُفتح مع ${s.id.startsWith("LINK") ? "مرحلة الربط" : "مرحلة النموّ"} — لا مفتاح هنا`,
      }),
