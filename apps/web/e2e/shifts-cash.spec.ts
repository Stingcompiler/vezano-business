  ...
  test("validation_error: خطآن يمنعان التسجيل — السحب يتجاوز الرصيد وسبب من كلمة واحدة", async ({
    ...
    await expectFrame(page, info, {
      ...
      texts: fromFrame("SHIFT-03", "validation_error", [
        ...
        ". الصندوق لا يصير سالباً بحركة يدوية — وإن كان فيه نقص فهو فرقُ عدّ يُراجَع في",
        "، لا سحبٌ يُسجَّل هنا.",
        "السبب",
    ...
    await expect(page.locator("body")).toContainText(
      "فهو فرقُ عدّ يُراجَع في «مراجعة فروق الورديات»، لا سحبٌ يُسجَّل هنا.",
    );
  ...
  test("success: أُقفلت الوردية — المعدود والمتوقَّع والفارق نقصاً، وشهادة العدّ", async ({
    ...
    await expectFrame(page, info, {
      ...
      texts: fromFrame("SHIFT-04", "success", [
        ...
        "الفارق",
        "نقصاً.",
        "يُسجَّل باسم الوردية لا باسم سالم شخصياً، ولا يُطالَب به تلقائياً. الفارق واقعةٌ تُراجَع في",
        "، والمطالبة قرار إنسان لا نتيجة حساب.",
        "شهادة العدّ",
        "عدَّ",
        "سالم — الكاشير",
        "الوقت",
    ...
    await expect(page.locator("body")).toContainText(
      "الفارق واقعةٌ تُراجَع في «مراجعة فروق الورديات»، والمطالبة قرار إنسان لا نتيجة حساب.",
    );
  ...
  test("partial: النقد عُدّ والبطاقة لم تُسوَّ — مقفلة نقداً · بطاقة معلّقة", async ({
    ...
    await expectFrame(page, info, {
      ...
      texts: fromFrame("SHIFT-04", "partial", [
        ...
        "ويُسوّى في",
        "حين يصل. الوردية «مقفلة نقداً · بطاقة معلّقة».",
      ]),
    });
    await expect(page.locator("body")).toContainText("ويُسوّى في «مراجعة فروق الورديات» حين يصل.");
