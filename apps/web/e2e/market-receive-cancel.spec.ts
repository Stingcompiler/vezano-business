  ...
  test("ready → partial → validation_error → conflict → success: العدّ لا يتجاوز المشحون، الرفض بسبب، والفارق خلاف لا تسوية", async ({
    ...
    await expectFrame(page, info, {
      ...
      texts: fromFrame("ORD-09", "ready", [
        ...
        "لماذا يُعرض المشحون",
        "لأنه دعوى الطرف الآخر لا رقمُ النظام (كما",
        "أنت تُقرّ بما وصلك مقابل ما يدّعي أنه أرسله.",
      ]),
    });
    ...
    await expect(root).toContainText(
      "هو يقول «شحنت 8»، وأنت تقول «استلمت 7». الرقمان يبقيان، ويُفتح خلاف في «الخلافات»",
    );
    ...
    await expectFrame(page, info, {
      ...
      texts: fromFrame("ORD-09", "validation_error", [
        ...
        "رفض خادمي لا تحذير",
        "الاستلام التراكمي لا يتجاوز المشحون",
        "الزيادة تُسجَّل بسبب مكتوب وتُحال إلى مراجعة الفرق.",
      ]),
    });
    ...
    await expectFrame(page, info, {
      ...
      texts: fromFrame("ORD-09", "conflict", [
        ...
        "دفتران مستقلان",
        "لا تسوية تلقائية ولا ترجيح",
        "كلٌّ يقيّد ما يقرّ به، والفرق محجوز حتى يُغلق.",
        "البابُ خلاف",
        "المسار المعروض «افتح خلافاً بالأدلة»",
        "لا «صحّح الكمية». تصحيحُ رقمٍ يخفي نزاعاً.",
      ]),
    });
    ...
    await expect(root).toContainText("SH-118 · الفارق 1 — فُتح خلاف الفارق");
  ...
  test("ready → validation_error → partial → success: ما يُلغى وما لا يُلغى، السبب مطلوب، الحوار هو الجدول، والحالة النهائية دقيقة", async ({
    ...
    await expectFrame(page, info, {
      ...
      texts: fromFrame("ORD-10", "ready", [
        ...
        "لا محو لما وقع",
        "المسلَّم والمسجَّل مالياً لا يُمسّ",
