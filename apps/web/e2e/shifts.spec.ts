  ...
  test("ready: تُفتح بعدٍّ — الوردية السابقة وما تركته، والوردية والفرع والجهاز", async ({
    ...
    await expectFrame(page, info, {
      ...
      texts: fromFrame("SHIFT-01", "ready", [
        ...
        "كاشير 2 — ديسكتوب",
        "المسؤول",
        "المعدود يطابق ما تركته الوردية السابقة. لو اختلف فليس خطأً يمنع الفتح — يُسجَّل الفارق باسم من عدّ ويُراجَع في",
        "، والبيع يبدأ الآن.",
    ...
    await expect(page.locator("body")).toContainText(
      "ويُراجَع في «مراجعة فروق الورديات»، والبيع يبدأ الآن.",
    );
