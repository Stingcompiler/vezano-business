  ...
  test("synced → success: مؤكد خادمياً، الطباعة الأولى نجاح، والثانية نسخة بنفس الرقم", async ({
    ...
    await expectFrame(page, info, {
      ...
      texts: fromFrame("POS-08", "synced", [
        ...
        "وصارت مرجعية.",
        "إعادة الطباعة نسخة",
        "كل طبعة بعد الأولى تحمل «نسخة» ورقمَ الأصل نفسه",
        ...RECEIPT,
        "طباعة الإيصال",
        "فتح الفاتورة",
        "بيع جديد",
  ...
  test("server_error: رُفض الرفع — البيع لا يُلغى", async ({ page }, info) => {
    ...
    await expectFrame(page, info, {
      ...
      texts: fromFrame("POS-08", "server_error", [
        ...
        "البيع لا يُلغى",
        "الرفض يذهب إلى مراجعة العمليات المتعثرة",
        "ولا يمسّ إيصالاً سُلّم. النقد قُبض والبضاعة خرجت — الورقة صادقة والمشكلة إدارية.",
        "محفوظ على الجهاز",
        ...RECEIPT,
        "طباعة الإيصال",
        "بيع جديد",
